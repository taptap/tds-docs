...
1. 参考 [准备工作](/v4/sdk/start/get-ready/) 创建应用、开启应用配置。
2. 参考[合规认证功能介绍](/v4/sdk/compliance/features/#准备工作)中准备工作开通防沉迷服务。
...
合规认证依赖于初始化和 TapTap 登录模块，使用本地文件方式添加依赖时，需先参考 [入门指南](/v4/sdk/start/quickstart#本地文件依赖)  和 [TapTap 登录](/v4/sdk/taptap-login/guide/#本地文件依赖) 添加对应本地文件依赖项。
...
合规认证模块支持设置 是否显示切换账号 和 使用年龄段信息，开发者可在 [TapSDK 初始化](/v4/sdk/start/quickstart#初始化) 时添加对应的配置，具体如下：
