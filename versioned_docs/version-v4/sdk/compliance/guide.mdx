...
由于TapSDK V4 依赖于TapTapCore核心库，所以需要在TapTapCore的基础上，另外添加 `TapTapCompliance` 、 `TapTapLogin` 模块：
...
3. 从解压缩中的 TapSDK 中，选择希望在应用中使用的 TapTapSDK 包导入。
    ...
    - `TapTapSDK_Compliance.unitypackage` 必选，TapTapSDK 合规认证模块
...
3. 将工程 Pods 目录下 `TapTapSDKCompilance/Frameworks/TapComplianceResource.bundle` 和 `TapTapLoginSDK/Frameworks/TapTapLoginResource.bundle`
   等资源文件导入工程中
...
防沉迷开始认证时需传入玩家唯一标识 `userIdentifier`，建议使用[TapTap 用户认证](/sdk/taptap-login/guide/tap-login/#taptap-登录并获取登录结果)
中的 `openid` 或 `unionid`。
...
获取方式可以参考[获取用户信息](/sdk/taptap-login/guide/start/#获取用户信息)的内容。
