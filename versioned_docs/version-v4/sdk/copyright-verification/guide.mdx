...
TapLicense.RegisterLicenseCallBack(ITapLicenseCallback callback);
...
```cs
TapLicense.CheckLicense();
TapLicense.checkLicense(true);
```
...
TapLicense.RegisterDLCCallback(new MyTapDLCCallback());
```
