...
[//]: # (<span style="color:#ff0000;">TODO android</span>)
</>
...
dependencies {
    // ...
    implementation 'com.taptap.sdk:tap-core:${sdkVersions.leancloud.java}'
}`}
</CodeBlock>
</>
...
如果项目中同时集成[内建账户系统的 TapTap 登录](/sdk/taptap-login/guide/start/#sdk-初始化)并完成了初始化，需要在此基础上添加 TapDBConfig 配置，即可同步初始化
TapDB。代码如下：
...
setEnable(true); //是否开启 TapDB
tapDBConfig.

setChannel("gameChannel"); //分包渠道，长度不大于 256
tapDBConfig.

setGameVersion("1.0.0"); //游戏版本，为空时，自动获取游戏安装包的版本，长度不大于 256
...
init(MainActivity .this, tapConfig);
```
...
init(getApplicationContext(), "clientId","taptap","gameVersion",true);
```
...
init(getApplicationContext(), "clientId","taptap","gameVersion",false);
```
...
onCharge("0xueiEns","轩辕剑","100","CNY","wechat",info);
```
...
put("#current_channel","TapDB");
TapDB.

registerStaticProperties(commonProperties);
```
...
unregisterStaticProperty("#current_channel");
```
