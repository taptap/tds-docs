...
本文介绍如何快速接入 TapSDK 并实现 [TapTap 登录](/v2/sdk/taptap-login/guide/start)功能。
...
上面代码示例中的 `AccessToken` 用于用户鉴权，过期时间为 90 天（过期后 SDK 会自动清除本地缓存），可以传到游戏服务端去获取用户信息，参见 [获取用户信息](/v2/sdk/taptap-login/guide/userinfo#流程)。
