...
请求的 Base URL 可以在**应用 > 设置 > 应用凭证 > 服务器地址**查看。
...
请求的鉴权是通过 HTTP Header 里面包含的键值对来进行的，详见《存储 REST API》中 [请求格式](/v3/sdk/storage/guide/rest/#请求格式) 一节的说明。
...
iOS 设备支持 `alert` 本地化消息推送，只需要将上面 `alert` 参数从 String 替换为一个由本地化消息推送属性组成的 JSON 即可：
...
我们也支持按照上述 Apple 官方文档的方式构造推送参数，例如：
...
where| 可选 | 检索 `_Installation` 表使用的查询条件，JSON 对象。如果查询条件内包含日期或二进制等需要做编码的特殊类型数据，查询条件内需要包含编码后的数据。如查询 `createdAt` 字段大于某个时间的设备，where 条件需要为 `{"createdAt":{"$gte":{"__type":"Date","iso":"2015-06-21T18:02:52.249Z"}}}`。更多信息请参看[数据存储 REST API 使用详解](/v3/sdk/storage/guide/rest/)的《数据类型》一节的说明。
...
_notificationChannel | 可选 | Android 8.0 以上设备在推送时需要传递 channel id 才能正常接收推送，请参看 [Android 推送指南](/v3/sdk/push/guide/android/)的《Android 8.0 推送适配》一节。
...
下面会举一些例子，更多例子请参考[数据存储 REST API 使用详解](/v3/sdk/storage/guide/rest/)的《查询》一节。
...
用 `where` 查询的都是 `_Installation` 表中的属性。这里假设该表存储了 `preOrder` 的布尔属性。
...
另外请注意，如果希望接收透传消息请不要忘记自行实现自定义 Receiver，参见 [Android 推送指南](/v3/sdk/push/guide/android/)的《自定义 Receiver》一节的说明。
...
可以配合云引擎的定时任务功能实现周期推送，非常方便。请阅读《在云引擎中使用推送服务》。
...
将返回推送记录对象，推送记录各字段含义参考[推送通知总览](/v3/sdk/push/guide/overview/)的《Notification》一节。

## 推送状态查看和取消

在发推送的过程中，我们会随着推送任务的执行更新推送状态到 **开发者中心 > 你的游戏 > 游戏服务 > 云服务 > 推送通知 > 推送记录** 中，可以在这里查看推送的最新状态。对不同推送状态的说明请参看[推送通知总览](/v3/sdk/push/guide/overview/)的《Notification》一节。
...
* <Conditional brand="leancloud">每个开发版应用处在待发队列中的定时推送数量最多 10 条，每个商用版应用处在待发队列中的定时推送数量最多 1000 条。</Conditional><Conditional brand="tds">每个应用处在待发队列中的定时推送数量最多 1000 条。</Conditional>
