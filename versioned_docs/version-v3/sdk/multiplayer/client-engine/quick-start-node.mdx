...
我们在这篇文档中先了解如何本地启动项目，简单试玩一下，然后部署项目到云端。之后在[开发指南](/v3/sdk/multiplayer/client-engine/guide-node/)中会介绍详细的游戏逻辑以及如何开发自己的游戏。

## 启动项目

### 安装命令行工具

请查看命令行工具**[安装部分](/v3/sdk/engine/cli/#安装)**的文档，安装命令行工具，并执行**[登录](/v3/sdk/engine/cli/#登录账号)**命令登录。
...
在浏览器中登录 LeanCloud 控制台，绑定[云引擎域名](/v3/sdk/domain/guide/#云引擎域名)（`stg-` 开头的自定义域名会被自动地绑定到预备环境），然后访问相应网址可以看到 Client Engine 服务端正在运行的文本。

其他详细的部署方式请参考命令行工具文档中的[部署](/v3/sdk/engine/cli/#部署)。

## 你的第一个 Client Engine 小游戏

接下来请查看文档[你的第一个 Client Engine 小游戏](/v3/sdk/multiplayer/client-engine/first-game-node/)，了解如何根据该初始项目一步一步开发出来一个剪刀石头布小游戏。
