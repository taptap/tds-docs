...
请阅读[数据存储 Java SDK 配置指南](/v3/sdk/storage/guide/setup-java/)。
...
comment.saveInBackground().subscribe(new Observer<LCObject>() {
        ...
        public void onNext(LCObject todo) {
            // 成功保存之后，执行其他逻辑
            Log.e(TAG, "保存成功。objectId：" + todo.getObjectId());
        }
        public void onError(Throwable throwable) {
            // 异常处理
        }
...
```xml
<service android:name="com.tapsdk.lc.push.PushService"/>
<receiver android:name="com.tapsdk.lc.push.LCBroadcastReceiver">
  <intent-filter>
...
请参阅[内建账户指南](/v3/sdk/authentication/guide/)。

## 角色

随着用户量的增长，你可能会发现相比于为每一名用户单独设置权限，将预先设定好的权限直接分配给一部分用户是更好的选择。为了迎合这种需求，云服务支持基于角色的权限管理。请参阅[ACL 权限管理开发指南](/v3/sdk/storage/guide/acl/)。
...
```java
// Student.java
import com.tapsdk.lc.LCClassName;
...
全文搜索是一个针对应用数据进行全局搜索的接口，它基于搜索引擎构建，提供更强大的搜索功能。要深入了解其用法和阅读示例代码，请阅[全文搜索指南](/v3/sdk/storage/guide/fulltext-search/)。
