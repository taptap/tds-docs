...
使用 TDS 的数据存储、云引擎要求必须绑定自有域名，其他服务可选是否绑定自有域名。
我们提供了 API 共享域名和文件共享域名，但是请注意，**共享域名没有可用性保证，容易受到 DDoS 攻击影响。我们建议在生产环境中使用绑定的自定义域名。**
...
参考 [存储 REST API 使用指南](/v3/sdk/storage/guide/rest/#base-url) 配置。

#### JavaScript SDK

##### 存储 SDK

请参考 [SDK 安装指南](/v3/sdk/storage/guide/setup-js/#安装与引用-sdk) 配置。
...
请参考 [即时通讯开发指南](/v3/sdk/im/guide/overview/) 配置。
...
请参考 [入门指南](/v3/sdk/multiplayer/start/js/#初始化) 或 [开发指南](/v3/sdk/multiplayer/guide/js/) 进行配置。
...
请参考 [SDK 安装指南](/v3/sdk/storage/guide/setup-objc/#初始化) 配置。
...
`>= 17.0.0` 的版本请参考 [SDK 安装指南](/v3/sdk/storage/guide/setup-swift/#初始化) 配置。
...
Java Unified SDK（`>= 6.0.0`）请参考 [SDK 安装指南](/v3/sdk/storage/guide/setup-java/#初始化) 配置。
...
请参考 [SDK 安装指南](/v3/sdk/storage/guide/setup-dotnet/#初始化) 配置。
...
[云引擎独立 IP](/v3/sdk/engine/dedicated-IP/) 可按需购买。
...
3. 当域名未备案时，会出现绑定失败的提示，这时需要对域名进行备案，可参考[域名备案指南](/v3/sdk/domain/icp/)，或者使用其他已备案的域名重新进行绑定。
...
```
因为工信部规定，网站接入多个云服务商时，需要在各云服务商处接入备案。这个报错是缺少对应的云服务商备案所导致的（如果是华东区的应用，很可能是缺少腾讯云备案）；可以参考[域名备案指南](/v3/sdk/domain/icp)进行备案操作；
...
可以参考视频教程：[如何绑定域名及如何提交备案](https://www.bilibili.com/video/BV1cu4y1V7mG/)。
