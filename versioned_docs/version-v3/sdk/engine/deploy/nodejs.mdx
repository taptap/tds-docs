...
:::info
这篇文档是针对 Node.js 运行环境的深入介绍，如希望快速地开始使用云引擎，请查看 [快速开始部署云引擎应用](/v3/sdk/engine/deploy/getting-started)。
:::
...
:::note
在使用 [命令行工具](/v3/sdk/engine/cli) 本地调试时，`lean up` 会优先使用 `npm run dev` 来启动项目。
:::
...
不过，如果项目的 Node.js 版本小于 10，或者项目目录下没有 lockfile，则会使用 `npm install --production` 安装依赖，相应地，`devDependencies` 中列出的依赖**不会**安装，可参考 [默认命令](/v3/sdk/engine/deploy/nodejs/#默认命令)。
