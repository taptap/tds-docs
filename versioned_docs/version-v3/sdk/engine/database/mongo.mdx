...
LeanDB MongoDB 是云引擎提供的托管数据库，开发者可以在云引擎中使用 MongoDB 客户端类库连接，访问完整的 MongoDB 功能，更多其他托管数据库请查看 [云引擎服务总览](/v3/sdk/engine/overview)。

## 创建和管理实例

开发者可以在 **<Path to="engine" /> > 数据库 > MongoDB** 页面创建和管理 LeanDB MongoDB 实例。
