...
* Android 的 [LCException](https://leancloud.github.io/java-unified-sdk/cn/leancloud/LCException.html) 以及 [常量字段值](https://leancloud.github.io/java-unified-sdk/constant-values.html#cn.leancloud.LCException.OTHER_CAUSE)。
...
* 含义 -  Web 服务器与后端应用服务器通讯失败，一般是某个应用服务器异常。应用服务器出现异常后，Web 服务器会在几秒后移除此实例，如果频繁遇到 524，请联系我们处理。另外云函数超时也有可能触发 524，云函数超时的解决方案请参考 [文档](/sdk/engine/functions/guides/#云函数超时)。
## 529
...
* 含义 - 无效的短信签名。具体要求请参考 [短信签名规范](/sdk/sms/guide/#签名规范)。

## 601

* 信息 - `Can't send SMS too frequently.`
* 含义 -  发送短信过于频繁。不同类型的短信有不同的 [发送数量限制及内容管控](/sdk/sms/faq/#短信有什么限制吗)。我们强烈建议用户使用图形验证码或者倒数计时等方式来避免用户重复发送验证码，以及可能存在的短信验证码攻击。
...
* 含义 - 缺少图形验证码功能要求的 token。请按照 [开通图形验证码服务](/sdk/sms/guide/#开通图形验证码服务) 来正确集成，或者在应用控制台中关闭「短信设置」里「强制短信验证服务使用图形验证码」选项。
