...
:::tip
服务通用规则请参考 [请求规则和签算](/docs/sdk/TapPayments/develop/server/#请求规则和签算)
:::
...
data 对象结构见 [订单信息](/docs/sdk/TapPayments/develop/server/#订单信息)
...
data.list 数组内对象结构见 [订单信息](/docs/sdk/TapPayments/develop/server/#订单信息)
...
推荐做法是：当商户系统收到通知时，先进行签名验证 [请求签算](/docs/sdk/TapPayments/develop/server/#请求签算)，然后检查对应业务数据的状态，如未处理，则进行处理；如已处理，则直接返回成功。
...
order 对象结构见 [订单信息](/docs/sdk/TapPayments/develop/server/#订单信息)
...
| **Header**  | **是否必须** | **描述**                                                     |
| ----------- | ------------ | ------------------------------------------------------------ |
| X-Tap-Sign  | Y            | 接口签算，详见 [请求签算](/docs/sdk/TapPayments/develop/server/#请求签算) |
...
| **字段**          | **类型** | **是否必须** | **描述**                                                     |
| ----------------- | -------- | ------------ |------------------------------------------------------------|
| code              | int      | Y            | [错误码](/docs/sdk/TapPayments/develop/server/#错误码)           |
| msg               | string   | Y            | 通用错误描述                                                     |
...
| user_region    | string   | Y            | [用户地区](/docs/sdk/TapPayments/appendix/regions-currencies) |
| goods_open_id  | string   | Y            | 商品唯一 ID                                                  |
| goods_name     | string   | Y            | 商品名称                                                     |
| status         | string   | Y            | 订单状态                                                     |
| amount         | string   | Y            | 金额（本币金额 x 1,000,000）                                 |
| currency       | string   | Y            | [币种](/docs/sdk/TapPayments/appendix/regions-currencies) |
