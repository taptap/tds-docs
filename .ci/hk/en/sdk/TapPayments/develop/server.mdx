...
#### 请求域名:
- `https://cloud.tapapis.com`

| ** 接口地址 **                      | **Method** | **描述**             |
| ----------------------------- | ---------- | -------------------- |
| `/payment/order/v1/info?client_id={{client_id}}&order_id={{order_id}}`        | `GET`        | 查询订单信息         |
| `/payment/order/v1/unconfirmed?client_id={{client_id}}` | `GET`        | 查询未核销的订单列表 |
| `/payment/order/v1/verify?client_id={{client_id}}`      | `POST`       | 核销订单             |
...
curl -X GET \
  ...
  https://{{domain}}/payment/order/v1/info?client_id={{client_id}}&order_id={{order_id}}
```
...
curl -X GET \
  ...
  -H 'X-Tap-Ts: {{unix timestamp}}' \
  -H 'X-Tap-Nonce: {{random nonce}}' \
  https://{{domain}}/payment/order/v1/unconfirmed?client_id={{client_id}}
```
...
curl -X POST \
  ...
  https://{{domain}}/payment/order/v1/verify?client_id={{client_id}}
```
...
#### 主查服务的密钥
- 密钥可在 **开发者中心 > 你的游戏 > 游戏服务 > 应用配置 > 基本信息 > Server Secret** 查看。
#### Webhook 回调的密钥
- 密钥可在 **TapTap 开发者中心 > 你的游戏 > 游戏服务 > TapPayment > 商品与订单 > API 密钥** 查看。
...
function getHeadersPart($headers) {
    ...
    foreach ($headers as $key => $value) {
        ...
        if (strtolower($key) === "x-tap-sign") {
            continue;
        }
        if (strpos($key, 'X-Tap-') === 0) {
            $signHeaders[strtolower($key)] = $value; // Assuming each header has a single value
        }
    }
    $headerKeys = [];
