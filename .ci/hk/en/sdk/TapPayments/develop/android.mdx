...
<CodeBlock className="java">
 {`dependencies {
    ...
    implementation 'com.taptap.android.payment:sdk:${sdkVersions.tapGlobalPayments.android}'
}
`}
</CodeBlock>
...
如果项目中有依赖 TapSDK 的库, 请使用 TapSDK 版本为 3.28.3 。

当项目中有引入 `TapCommon_X.XX.X.aar` 和 `TapLogin_X.XX.X.aar` 的离线包时，请删除本地引入的这两个包，然后**改成 Gradle 方式依赖**，打开项目的 `project/app/build.gradle` 文件，添加 gradle 配置如下：

```java
implementation("com.taptap.android:login:3.28.3@aar")
implementation("com.taptap.android:common:3.28.3@aar")
```
:::
...
这里需要设置对应在开发者后台申请的 `ClientID` 和 `ClientToken`，用于校验是否有权限使用 `TapPayment`。
