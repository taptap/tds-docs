...
* 信息 - `LOGIN_TIMEOUT`
* 含义 - 登录超时，连接后长时间没有完成 session open。通常是登录被拒绝等原因，出现此问题可能是使用方式有误，可以 [创建工单](https://leanticket.cn/t/leancloud)，由我们技术顾问来给出建议。
* 模块 - 即时通讯 IM
...
* 信息 - `INTERNAL_ERROR`
* 含义 - 服务器内部错误，如果反复出现请收集相关线索并 [创建工单](https://leanticket.cn/t/leancloud)，我们会尽快解决。
* 模块 - 即时通讯 IM
...
* 含义 - 对话操作引起服务器内部错误。如果反复出现请收集相关线索并 [创建工单](https://leanticket.cn/t/leancloud)，我们会尽快解决。
* 模块 - 即时通讯 IM
