...
点击事件管理右上角”新建事件“，选择”虚拟属性“。

![新建虚拟事件](/img/customEvent/virtualEvent_1.png)

### 3.1. 填写基础信息

填入虚拟事件名、虚拟事件显示名和说明，事件名默认以“#ve@”开头。

![填写基础信息](/img/customEvent/virtualEvent_2.png)
...
![管理虚拟事件](/img/customEvent/virtualEvent_4.png)

### 4.2. 分析模型中使用的注意事项

虚拟事件在使用上和通常的时间一致，其所关联的事件属性为所有基础属性的关联事件属性的交集

虚拟事件的触发用户数含有对基础事件联合去重的逻辑，因而在部分场景中，虚拟事件的触发用户数会小于所有基础事件触发用户数的直接加和

![分析模型中使用的注意事项](/img/customEvent/virtualEvent_5.png)
