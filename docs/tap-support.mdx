...
3. 前往客服后台，为当前游戏创建一个分类（**设置 > 管理 > 工单分类**），记下这个分类的 ID（配置分类需要客服角色，客服需要先在开发者中心邀请）。
...
TapSupport.Init("https://please-replace-with-your-customized.domain.com", "分类 ID");
```
...
TapSupportConfig config = new TapSupportConfig("https://please-replace-with-your-customized.domain.com", "分类 ID")
...
上述代码示例中，`please-replace-with-your-customized.domain.com` 就是准备工作 2 中获取或绑定的域名。`分类 ID` 是在准备工作 3 中新建分类得到的 ID。
...
```objc
NSDictionary *meta = @{@"243":@"iOS 15.1", @"244":@"WiFi"}; // 243 是在后台创建的「OS」字段的 ID，244 是在「角色名」字段的 ID
[TapSupport openSupportView:@"/" fieldsData:fields];
```
