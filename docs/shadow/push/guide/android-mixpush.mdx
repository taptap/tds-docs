...
可以参考这里的说明来发送推送请求：[推送 REST API 使用指南](/shadow/push/guide/rest/)。
...
1. 在 application 的 `onCreate` 方法中调用 `LeanCloud.initialize` 完成初始化之后，进行混合推送 library 的初始化：
    ...
    这里参数 `profile` 的用法可以参考[推送 REST API 使用指南](/shadow/push/guide/rest/)的《Android 混合推送多配置区分》一节。
...
- 参数 `profile` 的用法可以参考 [推送 REST API 使用指南](/shadow/push/guide/rest/)的《Android 混合推送多配置区分》一节。
