...
- Android 设备中的应用被杀掉，在网络中处于离线状态。对于这种情况，稍等一段时间待设备上线后，成功设备数就会逐渐增加。同时推荐集成 [混合推送](/shadow/push/guide/android-mixpush/) 来提升推送到达率。
...
可以。请参考 [消息推送开发指南](/shadow/push/guide/rest/#消息内容参数)。
...
iOS 能做到这点，是因为当应用进程关闭后，Apple 和设备的系统之间还会存在连接，这条连接跟应用无关，所以无论应用是否被杀掉，消息都能发送到 APNs 再通过这条连接发送到设备。但对 Android 来说，如果是国内用户，因为众所周知的原因，Google 和设备之间的这条连接是无法使用的，所以应用只能自己去保持连接并在后台持续运行，一旦后台进程被杀掉，就无法收到推送消息了。虽然 Android SDK 已经采取了各种办法保持应用在后台运行，但随着 Android 系统版本的升级，权限控制越来越严，第三方推送通道的生命周期受到较大限制。因此 Android SDK 推出了混合推送的方案，对接国内主流厂商，保障了主流 Android 系统上的推送到达率。详见 [Android 混合推送开发指南](/shadow/push/guide/android-mixpush/)。
...
解决办法是升级 SDK 版本到 v11.6.7 及以上，保存 deviceToken 的方法参见  [保存 Token](/shadow/push/guide/ios/#保存-token)。
...
关于 iOS 的推送环境参考：[推送环境](/shadow/push/guide/ios/#推送环境)。
...
示例代码可以参考：[iOS 消息推送开发指南](/shadow/push/guide/ios/#保存-token)。
...
_Installation 表记录了设备上生成的安装信息。可以在 _Installation 表新增一个字段来区分不同的应用。在推送消息的时候，如果选择推送全部用户，则两个应用都会收到推送。如果想要针对某一个应用推送，可以参考 [发送给特定的用户](/shadow/push/guide/android/#发送给特定的用户) 这个推送文档，只发送给指定的设备。

### _Installation 中的 valid 字段指的是什么，valid 为什么是 false？

valid 表示当前这条设备记录是否有效，是 false 表示这条记录失效了，比如长时间未使用推送或者推送设备并未登录。

比如 Android 设备未执行以下代码打开[启动推送服务](/shadow/push/guide/android/#启动推送服务)，valid 的值就会一直是 false。
...
所有经过 `/push` 接口发出的消息的都可以在控制台的消息菜单里的推送记录看到。每次调用 `/push` 都将产生一条新的 推送记录表示一次推送。该表的各属性含义请参考 [Notification 表详解](/shadow/push/guide/overview/#notification) 。
