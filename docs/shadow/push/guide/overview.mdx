...
使用推送服务前，需在 **应用 > 设置 > 安全中心** 开启推送服务，此开关有三分钟的延迟。
...
installationId|Android|SDK 为每个 Android 设备产生的唯一标识符
...
msg| |本次推送的消息内容，JSON 对象，详见[消息内容参数](/shadow/push/guide/rest#消息内容参数)。
...
<Conditional brand="tds">
注意：我们只保留最近一周的推送记录，并会对过期的推送记录定时进行清理。推送记录清理和推送消息过期时间无关，也就是说即使推送记录被清理，没有过期的推送消息依然是有效的，目标用户依然是能够收到消息。推送过期时间设置请参考[推送 REST API 使用指南](/shadow/push/guide/rest/)的《过期时间和定时推送》一节。
</Conditional>
...
<Conditional brand="tds">
请阅读 [Unity 推送指南](/shadow/push/guide/unity/)。
</Conditional>
...
<Conditional brand="tds">
请阅读 [Unreal 推送指南](/shadow/push/guide/Unreal/)。
</Conditional>

<Conditional brand="leancloud">
请阅读 [Unreal 推送指南](/sdk/push/guide/Unreal/)。
</Conditional>

## iOS 推送

<Conditional brand="tds">
请阅读 [iOS 推送指南](/shadow/push/guide/ios/)。
</Conditional>
...
<Conditional brand="tds">
详见 [Android 混合推送指南](/shadow/push/guide/android-mixpush/)。
</Conditional>
...
<Conditional brand="tds">
如果想要使用推送服务自有通道推送，请阅读 [Android 推送指南](/shadow/push/guide/android/)。
</Conditional>
...
<Conditional brand="tds">
请阅读[推送 REST API 使用指南](/shadow/push/guide/rest/)。
</Conditional>
