...
单独初始化悬浮窗需要优先完成`单纯 TapTap 认证` 和 `内嵌动态` 的初始化动作。关于单纯 TapTap 认证初始化[参考](/sdk/taptap-login/guide/tap-login/#初始化)，关于内嵌动态初始化[参考](/sdk/embedded-moments/guide/#单纯的内嵌动态初始化)，
