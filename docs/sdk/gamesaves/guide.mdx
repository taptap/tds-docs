...
2. 云存档需要依赖于内建账户，因此需要通过开发者中心开通 **TDS 内建账户**。游戏存档会绑定在 TDSUser 下，所以请首先参考[内建账户系统 Tap 登录](/sdk/taptap-login/guide/start/)文档完成[ SDK 获取](/sdk/taptap-login/guide/start/#sdk-获取) 、[SDK 初始化](/sdk/taptap-login/guide/start/#sdk-初始化)以及登录功能的接入。
