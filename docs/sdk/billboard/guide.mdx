...
Config.RegionType = static_cast<ERegionType>(StaticEnum<ERegionType>()->GetValueByNameString(CB_Init_RegionType->GetSelectedOption()));
Config.ServerURL = ""; // 开发者中心 > 你的游戏 > 游戏服务 > 基本信息 > 域名配置 > 云服务 API
Config.BillboardConfig = MakeShared<FBillboardConfig>();
Config.BillboardConfig->BillboardUrl = "";// 开发者中心 > 你的游戏 > 游戏服务 > 应用配置 > 域名配置 > 公告
Config.BillboardConfig->Dimensions = ETB_Init_Dimensions->MakeStringMap();    	
...
TapBillboard->OpenPanel(
		FSimpleDelegate::CreateUObject(this, &UServiceWidgetBillboard::OnOpenPanelSuccess),
		FTapFailed::CreateUObject(this, &UServiceWidgetBillboard::OnTapFailed),
		FSimpleDelegate::CreateUObject(this, &UServiceWidgetBillboard::OnBillboardClosed));
		
void UServiceWidgetBillboard::OnOpenPanelSuccess()
{
	TUDebuger::DisplayShow(TEXT("打开公告面板"));
}

void UServiceWidgetBillboard::OnTapFailed(const FTUError& Error)
{
	FString ErrorJsonString;
	FJsonObjectConverter::UStructToJsonObjectString(Error, ErrorJsonString);
	TUDebuger::ErrorShow(ErrorJsonString);
}
...
TapBillboard->OpenSplashPanel(
		FSimpleDelegate::CreateUObject(this, &UServiceWidgetBillboard::OnOpenSplashPanelSuccess),
		FTapFailed::CreateUObject(this, &UServiceWidgetBillboard::OnTapFailed),
		FSimpleDelegate::CreateUObject(this, &UServiceWidgetBillboard::OnSplashPanelClosed));

void UServiceWidgetBillboard::OnOpenSplashPanelSuccess()
{
	TUDebuger::DisplayShow(TEXT("Open splash panel success!"));
}
...
void UServiceWidgetBillboard::OnSplashPanelClosed()
{
	TUDebuger::DisplayShow(TEXT("Splash panel closed!"));
}
...
TapBillboard->GetBadgeDetails(
		FTapBadgeDetailsResult::CreateUObject(this, &UServiceWidgetBillboard::GetBadgeDetailSuccess),
		FTapFailed::CreateUObject(this, &UServiceWidgetBillboard::OnTapFailed));
		
void UServiceWidgetBillboard::GetBadgeDetailSuccess(const FBadgeDetails& BadgeDetails)
{
	const FString DebugStr = FString::Printf(TEXT("GetBadgeDetail: { ShowRedDot: %d }, { CloseButtonImg: %s }"), BadgeDetails.show_red_dot, *BadgeDetails.close_button_img);
	TUDebuger::DisplayShow(DebugStr);
}
...
```cpp
TapBillboard->RegisterCustomLinkListener(FOnCustomLinkClicked::FDelegate::CreateUObject(this, &UServiceWidgetBillboard::OnCustomUrlClicked));

void UServiceWidgetBillboard::OnCustomUrlClicked(const FString& Url)
{
	TUDebuger::DisplayShow(FString::Printf(TEXT("Custom url clicked: %s"), *Url));
}
...
```cs
[TapBillboard regsiterOutputStateListener:self];
- (void)onStopVoice{
    NSLog(@" billboard onStopVoice");
}

- (void)onPlayVoice{
    NSLog(@" billboard onPlayVoice");
}
```
...
```cpp
OutputStateChangedHandle = TapBillboard->RegisterOutputStateListener(FAudioOutputStateChanged::FDelegate::CreateUObject(this, &UServiceWidgetBillboard::OnOutputStateChanged));

void UServiceWidgetBillboard::OnOutputStateChanged(bool bPlaying)
{
	TUDebuger::DisplayShow(bPlaying ? TEXT("Audio Play") : TEXT("Audio Stop"));
}
