...
| 100010       | 超出服使用范围                                 | 提交的 ServerCode 错误，不在礼包设置的范围 |
...
| 100022       | 该区域不支持此兑换码                           | 非此 ClientId 的唯一兑换码                 |
...
A: 可以根据返回值中的 error 字段是否为 0 来判断。

Q: 无服务器兑换的 sign 字段如何使用？

A: 可以参考文档中[签名部分](/sdk/tds-gift/guide/#签名)。

Q: Unity 请求无服务器兑换接口报错 HTTP/1.1 422 Unprocessable Entity

A: 可以通过开发者后台 -> 礼包服务中检查活动开关是否打开，以及礼包码是否失效；
...
| 游戏方发送        |   兑换接口、礼包发放接口                |   游戏方仅需维护一个发放接口；可以通发放接口进行领取条件的判断以及礼包的发放       |
| 无服务器兑换      |   兑换接口                            |   仅判断对换码是否有效以及码量，对换码有效即返回礼包信息；流程更简单，无需自行维护接口       |