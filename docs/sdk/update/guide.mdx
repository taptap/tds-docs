...
TapTap 上游戏的**版本号**来自开发者在构建新版本时上传的 APK，唤起更新功能并不提供「检查 TapTap 商店是否有新版本」接口，游戏需自行实现判断版本的功能。
:::
...
* 在 [下载页](/tap-download) 找到 TapSDK Unity 下载地址，下载 TapSDK-UnityPackage.zip 然后解压，导入其中的 `TapTap_Common` 模块。
...
在 [下载页](/tap-download) 获得 TapSDK，添加 `TapCommon` 模块。
...
- 适用于中国大陆：`https://l.taptap.com/5d1NGyET?subc1=AppID`
...
- 适用于中国大陆：`taptap://taptap.com/app?app_id=AppID&source=outer|update`
