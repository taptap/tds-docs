...
假设你已经在 [快速开始](/sdk/start/quickstart/) 部分完成了[创建应用](/sdk/start/quickstart/#创建应用)、[项目配置](/sdk/start/quickstart/#项目配置) 和 [初始化](/sdk/start/quickstart/#初始化)。在 [下载页](/tap-download) 获得 TapSDK，另外添加 `TapFriends` 模块：
