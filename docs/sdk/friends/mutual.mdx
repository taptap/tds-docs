...
将用户加入黑名单，无论双方是否是好友，都可以进行该操作。拉黑后，双方之间进行中的好友申请都会被删除，且双方无法再发起以及接受对方的好友申请。查询好友列表时也无法查到已在黑名单中的好友，黑名单用户最多100人。
...
```java
public static void blockFriend(String userId, Callback<Void> callback)
```
...
参数 `userId` 为好友的 objectId, `callback` 为异步处理回调。
...
```cs
await TDSFriends.BlockFriend(userId);
```
...
```java
  TDSFriends.blockFriend(tomId, new Callback<Void>(){
            @Override
            public void onSuccess(Void result) {
            ...
            @Override
            public void onFail(TDSFriendError error) {
                System.out.println(" block tom fail error = " + error );            }
        });
```
...
```objc
[TDSFriends blockFriendWithUserId:userId callback:^(BOOL succeeded, NSError * _Nullable error) {
  if (succeeded) {
    // block user succeed.
  } else if (error) {
    // Failed to block.
...
```java
public static void unblockFriend(String userId, Callback<Void> callback)
```
...
参数 `userId` 为用户的`objectId`, `callback` 为异步处理回调。
...
```cs
await TDSFriends.UnblockFriend(userId);
```
...
```java
 TDSFriends.unblockFriend(id, new Callback<Void>() {
                @Override
                public void onSuccess(Void result) {
                    toast("unblock success");
                }

                @Override
                public void onFail(TDSFriendError error) {
                    toast("unblock fail error = " + error);
                }
            });
```
...
```objc
[TDSFriends unblockFriendWithUserId:userId callback:^(BOOL succeeded, NSError * _Nullable error) {
  if (succeeded) {
    // unblock succeed.
  } else if (error) {
    // Failed to unblock.
...
参数 `from` 为获取列表的起始位置，第一页为 0，下一页为上一次获取数据的总数量； 参数 `limit` 为每页获取数据的数量。

</>
<>

```java
public static void queryBlockList(int from, int limit, ListCallback<TDSFriendInfo> callback)
```

参数 `from` 为获取列表的起始位置，第一页为 0，下一页为上一次获取数据的总数量； 参数 `limit` 为每页获取数据的数量；参数`callback` 为异步处理回调，包含黑名单用户列表的数据信息。

</>
<>

```objc
+ (void)queryBlockListWithOption:(TDSFriendQueryOption *)option callback:(void (^)(NSArray<TDSFriendInfo *> * _Nullable, NSError * _Nullable))callback
```

参数 `option` 为查询时的条件配置，包括 `from` 每页的开始位置和 `limit` 查询数据数量等信息，callback 为异步处理结果回调。
