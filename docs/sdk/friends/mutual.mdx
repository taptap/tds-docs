...
[TDSFriends queryFriendRequestAndStateWithStatus:TDSUserFriendshipRequestStatusPending
    option:option
    callback:^(NSArray<TDSFriendQueryOption *> * _Nullable requests, NSError * _Nullable error) {
...
分享链接中传递的用户名称默认为玩家昵称（`nickname`），如果希望使用其他名称，可以在调用上述接口时另行指定。
...
TDSFriends.generateFriendInvitationLink("她姥姥", new Callback<String>() {
    // 略
});
```
...
NSString *inviteUrl = [TDSFriends generateFriendInvitationLinkWithError:option error:&error];
```
