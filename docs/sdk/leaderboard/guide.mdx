...
参考 [.NET SDK 配置](/sdk/storage/guide/setup-dotnet)。

</>
  
<>

参考 [Java SDK 配置](/sdk/storage/guide/setup-java)。
...
获取 TapTap 互关好友在游戏中的排行榜成绩列表（包括自己）。

<MultiLang kind="ue">

```cs
 ReadOnlyCollection<TDSLeaderBoardRanking> leaderBoards = await TDSLeaderBoardRanking.QueryTapFriendsLeaderBoard("world",0,10);
 string printData = "";
 foreach(TDSLeaderBoardRanking leaderBoard in leaderBoards){
      printData = printData + $"name = {leaderBoard.StatisticName} value = {leaderBoard.StatisticValue} rank = {leaderBoard.Rank} user = [ id = {leaderBoard.User.ObjectId} name = {leaderBoard.User["nickname"]} \n";
  }
 Console.WriteLine(printData);
```

```java
TDSLeaderBoardRanking.queryTapFriendsLeaderBoard("world", 0, 10, new Callback<List<TDSLeaderBoardRanking>>() {
    @Override
    public void onSuccess(List<TDSLeaderBoardRanking> result) {
        System.out.println("query success data = " + result);
    }
​
    @Override
    public void onFail(TapError error) {
        System.out.println("query fail error = " + error);
    }
});
```

```objc
 [TDSLeaderBoardRanking queryTapFriendsLeaderBoardWithName:@"world" from:0 limit:10 callback:^(NSArray<TapFriendLeaderBoard *> * _Nullable leaderboards, NSError * _Nullable error) {
        if(error){
        	NSLog(@" query fail error = %@",error.description);
        	return;
        }
        for(TDSLeaderBoardRanking *board in leaderboards){
           NSString *b = [NSString stringWithFormat:@" name = %@ value = %d rank=%d  user = [ id = %@ name = %@  \n", [board statisticName], [board statisticValue],
                           [board rank], [[board user] objectId], [[board user] valueForKey:@"nickname"]];
            print = [ print stringByAppendingString:b];
        }
        NSLog(@" detail = %@", print);
  }];
```
...
FTDSLeaderBoardRanking::FRankingsDelegate OnSuccess = FTDSLeaderBoardRanking::FRankingsDelegate::CreateLambda([](const TArray<FTDSLeaderBoardRanking>& Rankings) {
  for (auto Ranking : Rankings) {
    ...
    JsonObject->SetStringField("StatisticName", Ranking.StatisticName);
    JsonObject->SetNumberField("StatisticValue", Ranking.StatisticValue);
    JsonObject->SetNumberField("Rank", Ranking.Rank);
    JsonObject->SetObjectField("User",  TSharedPtr<FJsonObject>(Ranking.User->GetServerData()));
  }
});
FTUError::FDelegate OnError = FTUError::FDelegate::CreateLambda([](const FTUError& Error) {
    // handle error
});
FTDSLeaderBoardRanking::QueryTapFriendsLeaderBoard("world", 0, 10, OnSuccess, OnError);
```

</MultiLang>

参数 `name` 为排行榜的名称， `from` 和 `limit` 为分页获取数据时指定的起始位置和数据长度，返回为查询的结果数据，数据结构如下：

字段名称 | 字段内容 | 字段类型
--- | --- | ---
statisticName | 排行榜名称 | String
statisticValue | 排行榜中的成绩 | int
rank | 排行榜中的名次, 0 表示第一名，以此类推 | int
user | 排行榜数据对应的用户 | TDSUser
