...
## Tap 好友排行榜列表
获取 Tap 互关好友在游戏中的排行榜成绩列表（包括自己）。接口声明如下：
...
```cs
static async Task<ReadOnlyCollection<TDSLeaderBoardRanking>> QueryTapFriendsLeaderBoard(string name,
            int from, int limit)
```
...
```java
// callback 为查询结果的异步回调
 static void queryTapFriendsLeaderBoard(String name, int from, int limit, Callback<List<TDSLeaderBoardRanking>> callback) 
```
