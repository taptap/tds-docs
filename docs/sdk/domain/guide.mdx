...
| API 域名   | 存储<Conditional brand="tds">、排行榜、内建账户、好友、成就、云存档、实时语音</Conditional>、即时通讯<Conditional brand="leancloud">、短信</Conditional>、推送、**云函数** | 必须 | 必须                                                                                                               | 必须 | 应用     | 否           |
...
3. 我们推荐为 API 域名和云引擎域名配置独立 IP。**API 服务只对独立（IP）入口提供可用性保证。**未配置独立 IP 的云引擎服务默认会为静态站点优化，有一些对动态内容不那么友好的限制。详见下面的 [API 域名](#api-域名)、[云引擎域名](#云引擎域名)小节。
...
参考 [存储 REST API 使用指南](https://leancloud.cn/docs/rest_api.html#hash-1722650509) 配置。

#### JavaScript SDK

##### 存储 SDK

请参考 [SDK 安装指南](https://leancloud.cn/docs/sdk_setup-js.html#hash-99064366) 配置。
...
请参考 [入门指南](https://leancloud.cn/docs/multiplayer-quick-start-js.html#hash20935048) 或 [开发指南](https://leancloud.cn/docs/multiplayer-guide-js.html#hash20935048) 进行配置。
...
请参考 [SDK 安装指南](https://leancloud.cn/docs/sdk_setup-objc.html#hash20935048) 配置。
...
`>= 17.0.0` 的版本请参考 [SDK 安装指南](https://leancloud.cn/docs/sdk_setup-swift.html#hash20935048) 配置。
...
Java Unified SDK（`>= 6.0.0`）请参考 [SDK 安装指南](https://leancloud.cn/docs/sdk_setup-java.html#hash20935048) 配置。
...
请参考 [SDK 安装指南](https://leancloud.cn/docs/sdk_setup-dotnet.html#hash20935048) 配置。
