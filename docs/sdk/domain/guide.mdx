...
5. 开始进行域名解析，根据上一步推荐的 DNS 配置到你的域名注册商或域名解析服务提供商处设置进行域名解析（例如：你的域名是通过腾讯云或者阿里云购买的，这时就需要你通过腾讯云或阿里云管理后台中的域名管理处进行解析）。上图的第 5 步是以使用百度云服务商管理后台设置为例，大部分云服务商后台进行设置域名解析的路径大概为控制台 -> 域名管理 -> 解析，
