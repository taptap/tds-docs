...
| API 域名 | 存储、**云函数** | 必须 | 必须                  | 必须  | 应用   | 否      |
| API 域名 | 即时通讯、短信、推送 | 必须 | 可选                  | 必须  | 应用   | 否      |
...
| 云引擎域名  | 云引擎 **网站托管**                               | 必须 | 必须            | 可选  | 分组   | 视情况    |
| API 域名 | 存储、**云函数** | 必须 | 必须            | 必须  | 应用   | 否      |
| API 域名 | 排行榜、内建账户、好友、成就、实时语音、即时通讯、推送 | 必须 | 可选            | 必须  | 应用   | 否      |
...
那么建议你前往 **<Conditional brand="leancloud">应用控制台 > 设置 > 域名绑定 > API 访问域名</Conditional><Conditional brand="tds">开发者中心 > 游戏服务 > 应用配置 > 基本信息 > 域名配置 > API</Conditional>** 绑定 API 域名。
