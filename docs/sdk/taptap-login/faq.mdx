...
### 玩家长时间未登陆游戏 TapLoginHelper.getCurrentProfile().getOpenid() 获取不到 Openid 的问题 ？

TapTap 登录的授权信息会在玩家 30 天没打开过游戏（没有触发初始化）的前提下进游戏时会触发授权过期并自动清除相关数据。可以监听 Openid 值如果被清空了再走一遍[初始化](/sdk/taptap-login/guide/start/#sdk-初始化)流程，重新登陆，Openid 值重新登陆是不会改变的。
