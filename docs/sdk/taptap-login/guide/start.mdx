...
`unionid` | 通过用户信息加上厂商信息生成的**用户唯一标识**，一个玩家在一个厂商的所有游戏中 unionId 都是一样的，不同厂商下 unionId 不同

`openid` 和 `unionid` 使用标准的 Base64 （带 Padding）编码，包含的字符有 `A-Za-z0-9+/=`。

:::info
由于 `unionid` 与游戏所属厂商有强关联，一个游戏在[厂商转移](/store/store-creategame/#4-游戏主体可以进行转移吗)后同一个用户的 `unionid` 会发生改变。
