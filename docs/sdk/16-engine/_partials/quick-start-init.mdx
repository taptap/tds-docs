...
请先根据 [命令行工具使用指南 § 安装命令行工具](/sdk/engine/cli#安装命令行工具) 小节安装最新版本的命令行工具，然后根据 [命令行工具使用指南 § 登录](/sdk/engine/cli#登录) 小节登录到你的帐号。
...
<p><code>{CLI_BINARY} init</code> 会创建一个与你的应用同名的目录，我们 <code>cd myfunctions</code> 然后安装项目依赖：</p>
