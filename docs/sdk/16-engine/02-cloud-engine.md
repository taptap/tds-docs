...
部署到云引擎的应用只需遵循业界通用的项目结构（例如 Node.js 项目需要有 `package.json`），云引擎就可以自动地为其准备运行环境，可以在 [总览](/sdk/engine/overview) 中查看特定运行环境的文档来了解详情。
...
目前云引擎主要为无状态的 HTTP 服务而优化，不支持在文件系统上持久化地存储数据，应用可以将数据存储到 [数据存储](/sdk/storage/features) 或 LeanDB 提供的 [Redis](/sdk/engine/database/redis/)、[MongoDB](/sdk/engine/database/mongo/) 和 [Elasticsearch](/sdk/engine/database/es/) 中。
...
如需直接部署生产环境可以加上 `--prod 1` 这个参数，更多命令行工具的用法请查看 [云引擎命令行工具使用指南](/sdk/engine/cli)。
...
云引擎支持 SSH 协议的私有仓库（如 `git@github.com:leancloud/node-js-getting-started.git`），你需要在设置 Git 仓库地址时在 Git 托管服务处为云引擎配置 deploy key。如果你的仓库是公开的，则推荐使用 HTTPS 形式的仓库地址。
...
每个分组都可以部署云函数、Hook 和定时任务。但如果当前部署的代码中有和其他组同名的云函数，会中断部署，需勾选[「强制覆盖同名云函数」选项](#部署选项)。
...
将实例分组可以实现在访问同一数据源的情况下，部署多份云引擎代码，满足不同的业务需求。每个组可以绑定独立域名。详见 [分组管理](#分组管理)。
...
为了防止实例因为资源使用超限而受到影响，我们建议开发者经常 [查看统计图表](#查看统计图表)：
