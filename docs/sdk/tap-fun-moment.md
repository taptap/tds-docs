...
## 5. 获取用户新通知数量
<Highlight color='#f00'> 小红点功能对提升使用率至关重要，建议将入口放在显眼的位置。API调用频率为1次/分钟</Highlight>  

**显示小红点逻辑**  
FetchNotification 获取消息通知，如果不为0，游戏可以在动态入口显示小红点  

**关闭小红点逻辑** 
- 当调用 Open 打开动态后，游戏需要将UI上的小红点 disable，表示已读。
- 当游戏定时任务【每分钟调用FetchNotification】获取到小红点数量为0时，需要 disable 小红点
