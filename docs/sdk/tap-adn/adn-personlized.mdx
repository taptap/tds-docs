...
为落实个人信息保护相关的规定，TapADN SDK 为开发者提供退出个性化广告能力的接口，开发者可以调用接口，向用户提供退出个性化广告的能力。退出后，看到的广告数量不变，相关度会降低。开发者需遵守相关法律法规的要求，在开发者应用内为用户提供退出个性化广告的功能，保证在用户点击退出功能后调用穿山甲SDK的能力接口。
