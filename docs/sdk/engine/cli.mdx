...
也可以选择编译好的绿色版 **exe** 文件，下载后将此文件更名为 <Command suffix=".exe" />，并将其路径加入到系统 **PATH** 环境变量（[设置方法](https://www.java.com/zh-CN/download/help/path.html)）中去。这样使用时在 Windows 命令提示符（或 PowerShell）下，在任意目录下输入 <Command /> 就可以使用命令行工具了。当然也可以将此文件直接放到已经在 PATH 环境变量中声明的任意目录中去，比如 `C:\Windows\System32` 中。
...
| `file`    | 上传文件至数据存储服务（可以在 **<Path to="storage" /> > 文件** 中查看） |
| `logs`    | 显示云引擎日志                                                           |
...
默认部署备注为「从命令行工具构建」，显示在 **<Path to="engine" /> > 云引擎分组 > 日志** 中。你可以通过 `-m` 选项来自定义部署的备注信息：
...
使用命令 <Command suffix=" preview deploy" /> 来部署预览环境，在 GitLab CI 和 GitHub Actions 中会自动读取 PR 号、分支名等信息，若使用其他 CI 或在本地部署，需要手动指定。

我们提供了 GitLab CI 和 GitHub Actions 的模版：

<details>
<summary>点击展开 GitLab CI (<code>.gitlab-ci.yml</code>) 模版</summary>

在 GitLab 左侧点击 **CI/CD > Editor** 新建一个 `.gitlab-ci.yml` 文件并添加以下内容。注意替换 `YOUR_REGION` 和 `YOUR_APP_ID` 为相应的区域和 App ID，并在 GitLab 的 **Settings > CI/CD > Variables** 中添加你的 ACCESS_TOKEN。

<CodeBlock className="yaml">
{`before_script:
  - apt-get update && apt-get install -y curl
  - curl -L -o /bin/${CLI_BINARY} http://releases.leanapp.cn/leancloud/lean-cli/releases/download/v1.1.0/${CLI_BINARY}-linux-x64
  - chmod +x /bin/${CLI_BINARY}\n
deploy_preview:
  stage: deploy
  script:
    - ${CLI_BINARY} login --region YOUR_REGION --token $ACCESS_TOKEN
    - ${CLI_BINARY} switch --region YOUR_REGION --group web YOUR_APP_ID
    - ${CLI_BINARY} preview deploy
  environment:
    name: preview/$CI_COMMIT_REF_NAME
    url: TODO_FILL_THIS_URL
...
delete_preview:
  stage: deploy
  script: ${CLI_BINARY} preview delete
  environment:
    name: preview/$CI_COMMIT_REF_NAME
    action: stop
  rules:
...
<details>
<summary>点击展开 GitHub Actions 模版</summary>

在 GitHub 打开 **Settings > Secrets and variables > Actions** 中创建一个 secret，添加你的 ACCESS_TOKEN。
...
env:
  REGION: MY_REGION # set this to your ${BRAND == 'tds' ? 'TDS' : 'LeanCloud'} region, e.g. ${BRAND == 'tds' ? 'cn-tds1' : 'cn-n1'}
  APP_ID: MY_APP_ID # set this to your App ID on ${BRAND == 'tds' ? 'TDS' : 'LeanCloud'}
  ...
  deploy-preview-environment:
    ...
    steps:
      ...
      - name: Install lean-cli
        run: |
          sudo curl -L -o /bin/${CLI_BINARY} http://releases.leanapp.cn/leancloud/lean-cli/releases/download/v1.1.0/${CLI_BINARY}-linux-x64
          sudo chmod +x /bin/${CLI_BINARY}\n
      - name: Deploy
        run: |
          ${CLI_BINARY} login --region \${{ env.REGION }} --token \${{ secrets.ACCESS_TOKEN }}
          ${CLI_BINARY} switch --region \${{ env.REGION }} --group \${{ env.GROUP }} \${{ env.APP_ID }}
          PREVIEW_URL=\$(${CLI_BINARY} preview deploy)
...
- 设置 git repo 地址以及 deploy key 的方法可以参考 [云引擎平台功能 § Git 部署](/sdk/engine/platform/#git-部署)。
...
- [macOS 使用 `lsof` 和 `kill`](https://stackoverflow.com/questions/3855127/find-and-kill-process-locking-port-3000-on-mac)
- [Linux 使用 `fuser`](https://stackoverflow.com/questions/11583562/how-to-kill-a-process-running-on-particular-port-in-linux)
- [Windows 使用 `netstat` 和 `taskkill`](https://stackoverflow.com/questions/6204003/kill-a-process-by-looking-up-the-port-being-used-by-it-from-a-bat)
...
- <Command suffix=" up" /> 默认不再自动拉取线上的环境变量，如果需要的话可以手动添加
  `--fetch-env` 参数。
