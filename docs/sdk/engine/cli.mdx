...
预览环境目前只支持在 CI 中使用命令行工具部署，后续会支持不用配置 CI 自动同步的功能。
...
<details>
<summary>点击展开 GitHub Actions (<code>.github/workflows/main.yml</code>) 模版</summary>

在 GitHub 点击 **Actions > set up a workflow yourself** 新建一个 `.github/workflows/main.yml` 文件并添加以下内容。 注意替换 `YOUR_REGION` 和 `YOUR_APP_ID` 为相应的区域和 App ID，并在 GitHub 的 **Settings > Secrets and variables > Actions** 中添加你的 ACCESS_TOKEN。
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Deploy Preview
        run: |
          ${CLI_BINARY} login --region YOUR_REGION --token \${{ secrets.ACCESS_TOKEN }}
          ${CLI_BINARY} switch --region YOUR_REGION --group web YOUR_APP_ID
          ${CLI_BINARY} preview deploy\n
  delete:
    if: github.event.action == 'closed'
