...
在此基础上，开发者可以选择在程序内接入云引擎的 SDK，来使用云函数和 Hook 等功能，云函数与 <Conditional brand="tds">[数据存储](/sdk/storage/features)</Conditional><Conditional brand="leancloud">[数据存储](https://leancloud.cn/docs/storage_overview.html)</Conditional> 服务有深度的整合，对于已经在使用数据存储服务的开发会非常方便。
