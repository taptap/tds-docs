...
TapAdConfig config = new TapAdConfig.Builder()
                .AppId("1000007")               // 后台获得 AppId
                .AppName("联盟测试-Android")     // 后台 AppName
                .Key("1AjDOjD0F3SDDmgTuBQHbCRULSizYPHV17viZObHvhDjf7Pq1rlarueOX1cYBucn") // 后台获得 App 加密 key
                .Version("1")       // 版本
                .Channel("taptap2") // 渠道
                .Build();
// highlight-end
...
for (int i = 0 ; i < 3; i ++) {
    UserAction tmp = new UserAction.Builder()
        .ActionType(i)          // 自定义类型
        .ActionTime((long) (DateTime.UtcNow - jan1st1970).TotalMilliseconds)    // 上报时间
        .Amount(i * 1000)   // 数量
        .WinStatus(i % 2)   // TODO
        .Build();
    userActions[i] = tmp;
