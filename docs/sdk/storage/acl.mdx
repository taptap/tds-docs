...
<Conditional brand="tds">TDS </Conditional><Conditional brand="leancloud">LeanCloud</Conditional> 云端使用的 ACL 机制是将每个操作授权给特定的 User 用户或者 Role 角色，只允许这些用户或角色对一个对象执行这些操作。例如如下 ACL 定义：
...
使用内建数据表 `_User` 来维护 [用户/账户系统](/sdk/authentication/guide/)，以及内建数据表 `_Role` 来维护**角色**。
...
你可以设置 read 和 write 权限开放给哪些用户，其中「所有用户」、「指定用户」可以参考数据安全指南的 [Class 层面的访问权限](/sdk/storage/guide/security/#class-权限)一节的说明。
