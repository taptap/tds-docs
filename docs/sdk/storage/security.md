...
进入**开发者中心 > 你的游戏 > 游戏服务 > 云服务 > 数据存储 > 结构化数据**，选择一个 Class，再点击**权限**标签页。
...
在控制台还可以设置每个字段的权限。
访问 **开发者中心 > 你的游戏 > 游戏服务 > 云服务 > 数据存储 > 结构化数据**，选择一个 Class，点击相应字段的下拉菜单，选择 **编辑**：
...
**开发者中心 > 你的游戏 > 游戏服务 > 云服务 > 数据存储 > 服务设置 > 安全设置** 中可以设置是否开启 LiveQuery。

出于安全考虑，有些应用只在服务端调用接口向用户推送通知。
对于这些应用，可以访问 **开发者中心 > 你的游戏 > 游戏服务 > 云服务 > 推送通知 > 设置** 勾选 **禁止从客户端进行消息推送**。
勾选后客户端无法推送消息，只能通过 MasterKey 调用 REST API 发送或在控制台发送（**开发者中心 > 你的游戏 > 游戏服务 > 云服务 > 推送通知 > 在线发送**）。





**开发者中心 > 你的游戏 > 游戏服务 > 云服务 > 数据存储 > 服务设置 > 安全设置** 下可以设置 **禁止客户端创建 Class**。有些应用的开发者习惯预先规划好应用需要用到的 Class（表），并事先在控制台创建相应的 Class。对于这些开发者，推荐始终勾选这一选项。有些应用的开发者习惯直接着手开发应用的原型，在开发过程中逐渐完善数据结构。对于这些开发者，我们推荐在开发测试阶段不勾选这一选项，在应用上线前勾选这一选项。

**开发者中心 > 你的游戏 > 游戏服务 > 云服务 > 数据存储 > 服务设置 > 查询设置** 下可以设置 **查询 include 引入的 Pointer 类型数据时校验 ACL 权限**。我们建议所有应用都勾选这一选项（这一选项默认处于勾选状态），以保证数据安全。

**开发者中心 > 你的游戏 > 游戏服务 > 云服务 > 内建账户 > 设置** 页面下有一些用户相关的安全选项，可以要求修改密码时提供旧密码，密码修改后强制重新登录，验证第三方登录的 `Access Token` 是否有效。

**开发者中心 > 你的游戏 > 游戏服务 > 云服务 > 数据存储 > 文件 > 设置** 可以限制用户上传的文件类型。
