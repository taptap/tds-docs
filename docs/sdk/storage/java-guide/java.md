...
进入 **开发者中心 > 你的游戏 > 游戏服务 > 云服务 > 数据存储 > 服务设置**，在 **安全设置** 里面勾选 **启用 LiveQuery** 即可。确保即时通信模块已被添加到 `AndroidManifest.xml`：
...
取决于游戏的具体需求，也可以考虑在 **开发者中心 > 你的游戏 > 游戏服务 > 云服务 > 数据存储 > 文件 > 权限** 向特定用户或特定角色开启删除权限。
