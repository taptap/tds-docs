...
- **AntiAddiction:** 调整 iOS 桥接函数命名，避免与项目侧冲突

## 3.19.3
更新时间 2023-08-16

### Bug 修复
- **Common:** 修复 Android 当使用网页进行实名授权且通过返回键取消时没有回调的问题
- **Common:** 适配 Android 11 存储权限判断
- **Moment:** 适配 Android 13 相册读取权限及修改保存至相册时权限请求
- **TapDB:** 适配 Android 11 存储权限判断
- **TapDB:** 修复 SetUser 更新属性错误的 bug
- **Support:** 适配 Android 文件存储权限判断

### 功能更新
- **Billboard:** 支持缺省海外域名配置
...
- **Billboard:** 修复 Android 部分机型显示公告异常的兼容性问题
...
### Bug 修复
- **Login:** Android Tap 登录修复一些特殊情况下的崩溃（增加了 null 检查避免 NPE ）
...
### Bug 修复
- **Common:** 修复 TapSDK 初始化时未传公告参数产生的 NPE
...
- **DB:** 桥接传入参数为 null 的兼容处理
- **DB:** 接口优化
- **AntiAddiction:** iOS平台 传入的 userId 包含特殊字符时的兼容处理
...
- **Moment:** 新增场景化回调接口
  ...
  ```
- **DB:** Android 新增游戏 TapTap 共享 ID 开关
...
- **Moment:** Android 在部分刘海屏设备上判断失败导致 UI 可能被遮挡的问题
