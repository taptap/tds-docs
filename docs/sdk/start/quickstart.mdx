...
2. 编辑 `Assets/Plugins/Android/AndroidManifest.xml` 文件，在 `application` 标签下添加以下代码。
...
| 服务 | 需要的 SDK 包 | 
|---|---|---|
| TapTap 登录、内建账户 | TapBootstrap、TapCommon、TapLogin |
| 数据分析  | TapBootstrap、TapCommon、TapDB |
| 内嵌动态 | TapBootstrap、TapCommon、TapLogin、TapMoment |
| 游戏好友 | TapBootstrap、TapCommon、TapFriend |
| 成就系统 | TapBootstrap、TapCommon、TapAchievement|
| 公告系统 | TapBootstrap、TapCommon、TapBillboard |
| 防沉迷 | TapCommon、AntiAddiction、AntiAddiction |
| 数据存储、排行榜| cn.leancloud:storage-android、io.reactivex.rxjava2:rxandroid |
| 实时语音| TapRTC |
| 即时通信与推送| cn.leancloud:realtime-android、io.reactivex.rxjava2:rxandroid|
| 云存档| TapBootstrap、TapCommon、TapLogin、cn.leancloud:storage-android、io.reactivex.rxjava2:rxandroid |
...
为了正确填写签名 MD5 值，可以使用如下的工具进行获取：[**GenSignatureMD5**](https://blog.taptap.dev/tools/get-signature-md5.apk)。
