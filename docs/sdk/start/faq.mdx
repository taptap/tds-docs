...
当只有 APK 文件包时，为了正确填写签名 MD5 值，可以使用如下的工具进行获取：[**GenSignatureMD5**](https://lc-buhezimj.cn-e1.lcfile.com/PlrDdO71uiuAO7BVCYIP5Bvws8ksrN7G/tds_getsign.apk)，工具使用方式：使用正式的签名证书对游戏应用进行签名打包，然后将 APK 包安装到手机上。与此同时，将 GenSignatureMD5 工具也安装到同一部手机上，然后打开该工具输入游戏包名就可以得到签名 MD5 值。
