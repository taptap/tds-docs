...
| 回调 code | 回调类型 | 触发逻辑 |
|---|---|---|
| 500 | `LOGIN_SUCCESS` | 玩家未受到限制，游戏正常进入 |
| 1000 | `EXITED` | 退出防沉迷认证及检查，当开发者调用 Exit 接口时触发 |
...
| 1100 | `AGE_LIMIT` | 当前用户因年龄限制无法进入游戏，当 SDK 执行实名状态拦截器内方法返回 false 时触发
| 9002 | `REAL_NAME_STOP` | 实名过程中点击了关闭实名窗，游戏可重新开始防沉迷认证 |

### 初始化

从 `3.27.0` 版本开始，防沉迷初始化有两种方式，使用 `TapBootstrap` 模块（推荐） 和 单独调用防沉迷接口初始化，游戏根据需要选择一种即可， `3.27.0` 之前的版本只支持单独调用防沉迷接口初始化。
...
```cs
TapAntiAddictionConfig config = new TapAntiAddictionConfig()
{
...
tapAntiAddictionConfig.setNeedAgeRange(true);
...
```objc
 ...
 config.tapAntiAddictionConfig.needAgeRange = true;
...
Config.TapAntiAddictionConfig->bNeedAgeRange = true;
...
<>
 ...
 - `needAgeRange` 游戏是否需要获取真实年龄段信息，当设置为 true 时，使用 Tap 实名时会需要用户主动授权，当设置为 false 时，使用 Tap 实名时会进行无 UI 交互的静默授权，未设置时默认为 true
...
因防沉迷模块依赖于 `TapLogin` 模块，所以在防沉迷模块初始化前必须完成 `TapLogin` 模块[初始化](/sdk/taptap-login/guide/tap-login/#初始化)初始化。
...
Config config = new Config.Builder()
  ...
  .build();
//设置配置及回调， callback 为开发者实现的自定义防沉迷回调对象
AntiAddictionUIKit.init(activity, config, callback);
```
...
[AntiAddiction initWithConfig:config delegate:delegate];
```
</>
...
防沉迷开始认证时需传入玩家唯一标识 `userIdentifier`，如果接入 [TDS 内建账户系统](/sdk/taptap-login/guide/start/#一键完成-taptap-登录)，可以用玩家的 `objectId`；如果使用[单纯 TapTap 用户认证](/sdk/taptap-login/guide/tap-login/#taptap-登录并获取登录结果)则可以用 `openid` 或 `unionid`。
...
2. 在防沉迷初始化时配置的参数 needAgeRange 设置为 false 导致，需设置为 true
...
当除了需要满足防沉迷政策外，应用需要对用户年龄有额外限制，例如只允许 16 周岁以上使用时，开发者可在 DC 页面配置对应的年龄限制Hi，SDK 将在用户完成实名后 且 根据时长限制规则显示 UI 前 检查用户是否符合游戏要求，满足要求时，SDK 会继续进行后续时长业务及回调处理，否则会直接返回 code 为 `1100` 的年龄限制回调通知开发者。
