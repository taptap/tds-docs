...
:::info
* 防沉迷 v1.2.0 之后的版本并入了 TapSDK 的大版本，v1.2.0 之后的第一个版本为 v3.7.1。
* 防沉迷 v1.2.0 及之前版本仅需要导入**防沉迷 SDK**，不依赖其他 SDK。
:::
...
3.x.x 版本使用示例，在项目的 `Packages/manifest.json` 文件中添加以下依赖：
...
1.2.0 版本使用示例，在项目的 `Packages/manifest.json` 文件中添加以下依赖：
...
**注意**：导入 SDK 时请注意选择集成的版本，3.x.x 版本和 1.2.0 版本需要引入的依赖库文件数是不同的。

在 Unity 顶部菜单中选择 **Window > Package Manager** 可查看已经安装在项目中的包。

#### 方法二：手动导入

手动导入方式需要导入：`Common` 模块和 `storage` 模块
...
:::info
防沉迷 1.2.0 及之前版本 sdk 必须接入 TapTap 登录才能使用 TapTap 快速认证。
:::
...
如果 SDK 检测到玩家设备中未安装 TapTap 客户端，则会打开 WebView，玩家可授权游戏使用其在 TapTap 上的年龄段信息来完成游戏内的实名流程。
