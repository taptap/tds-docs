...
从 `3.27.0` 版本开始，防沉迷初始化有两种方式，使用 `TapBootstrap` 模块（推荐）初始化和单独调用防沉迷初始化接口，游戏根据需要选择一种即可， `3.27.0` 之前的版本只支持单独调用防沉迷初始化接口。
...
因为这两种流程都包含了 TapTap 快速认证的流程，所以**必须要开通 TapTap 登录服务**。
