...
开启测试模式的接口需要在 `init` 初始化之后，`startup` 启动之前调用。而且无论前面接入的是 [TDS 内建账户系统](/sdk/taptap-login/guide/start/#一键完成-taptap-登录)，还是使用[单纯 TapTap 用户认证](/sdk/taptap-login/guide/tap-login/#taptap-登录并获取登录结果)，在 `startup` 传入的 **玩家唯一标识** `userIdentifier` 必须为 TapTap 用户的 unionid，获取方式可以参考[获取用户信息](../../taptap-login/guide/start/#获取用户信息)的内容。
