...
<ul>
    <li>TapSDK-UnityPackage.zip 解压后，导入其中的 <code>TapTap_Common_{sdkVersions.taptap.unity}.unitypackage</code> 和 <code>TapTap_AntiAddiction_{sdkVersions.taptap.unity}.unitypackage</code>以及<code>TapTap_Login_{sdkVersions.taptap.unity}.unitypackage</code>。</li>
</ul>
...
dependencies {
    ...
    implementation(name: "TapBootstrap_${sdkVersions.taptap.ios}", ext: "aar") // 可选
    // ...
}`}
</CodeBlock>
...
AntiAddictionUIKit.StartupWithTapTap(userIdentifier, true);
```
