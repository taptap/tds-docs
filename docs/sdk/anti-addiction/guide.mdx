...
从 `3.27.0` 版本开始，防沉迷初始化有两种方式，使用 `TapBootstrap` 模块（推荐）初始化和单独调用防沉迷初始化接口，游戏根据需要选择一种即可，旧版本只支持单独调用防沉迷初始化接口。
...
```cs
var configBuilder = new TapConfig.Builder()
        .ClientID(initSetting.clientId)
        .ClientToken(initSetting.clientToken)
        .ServerURL(serverUrl)
        .RegionType(RegionType.CN);
configBuilder = configBuilder.AntiAddictionConfig(showAntiAddictionSwitchAccount);        
...
```cs
Action<int, string> ChinaAntiAddictionCallback => (code, s) =>
            TapLog(string.Format("放防沉迷回调 code: {0} 错误Msg: {1}", code, s), isReplace: false);

AntiAddictionUIKit.SetAntiAddictionCallback(ChinaAntiAddictionCallback);
```
...
```cpp
AntiAddictionUE::OnCallBack.BindLambda([Printer](AntiAddictionUE::ResultHandlerCode Code, const FString& Message)
	{
		TMap<AntiAddictionUE::ResultHandlerCode, FString> DesMap = {
			{AntiAddictionUE::LoginSuccess, TEXT("登录成功")},
			{AntiAddictionUE::Exited, TEXT("用户登出")},
			{AntiAddictionUE::SwitchAccount, TEXT("切换账号")},
			{AntiAddictionUE::DurationLimit, TEXT("时长耗尽，用户当前无法进行游戏")},
			{AntiAddictionUE::PeriodRestrict, TEXT("宵禁时间，用户当前无法进行游戏")},
			{AntiAddictionUE::RealNameStop, TEXT("取消实名")}
		};
		FTapApiNode OnCallbackNode = Printer.NewCallback(TEXT("OnCallBack"));
		OnCallbackNode[TEXT("Code")] = Code;
		OnCallbackNode[TEXT("CodeDescription")] = DesMap[Code];
		OnCallbackNode[TEXT("Message")] = Message;
		Printer.Print();
	});
