...
注意，为防止由于大量证书错误所产生的性能问题，我们对使用 **开发证书** 的推送做了设备数量的限制，即一次至多可以向 20,000 个设备进行推送。如果满足推送条件的设备超过了 20,000 个，系统会拒绝此次推送（**云服务控制台 > 推送 > 推送记录** 页面会显示相应信息）。因此，在使用开发证书推送时，请合理设置推送条件。
...
订阅后要记得保存，即可在 **云服务控制台 > 数据存储 > 结构化数据 > `_Installation`** 中看到该 installation 的 channels 字段多了一个「Giants」。
