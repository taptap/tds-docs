...
如果只需要单独使用 TapDB，可以只导入依赖 `common` 和 `tapdb`。其中 Unity v3.7.1 及更高版本，还需要导入 `com.leancloud.storage`（iOS SDK 及 Android SDK 不需要增加这一步）。
...
AdService.framework | 广告框架 | 设置为 optional
...
初始化 SDK 并上报一个设备登录（ `device_login` ）事件，调用这个接口是使用其它接口的先决条件，需要尽早调用。
...
<Conditional region='global'>
国际版应用初始化时，isCN 参数传入 `false`，表示中国大陆以外的国家或地区。
</Conditional>
...
> 注意：SDK 版本 3.15.0 及以上支持 OAID 版本为 1.0.5 ~ 1.2.1； 3.14.0 及以下支持 OAID 版本为 1.0.5 ~ 1.0.25


TapDB SDK 在应用接入 OAID 第三方库时，会在发送相关事件中携带该参数（key 为 `device_id4`)。现支持该第三方库版本为 1.0.5 ~ 1.2.1，因不同版本变更较大，所以针对不同版本接入的说明如下：

对于 1.0.5 ~ 1.0.25 不需要额外配置，只需应用添加对应第三方库的依赖即可。


对于 1.0.26 ~ 1.2.1 除添加对应第三方库外，需要添加如下处理：

#### 1. 在应用 assets 目录添加证书及配置文件

证书文件为应用通过 移动安全联盟邮箱 <msa@caict.ac.cn> 申请的 `.cert.pem` 文件, 该文件与包名对应。需要将该文件拷贝到应用 `assets` 目录，并注意该文件名应设置为 `packageName.cert.pem` , `packageName` 为当前应用包名。
...
* 在 [下载页](/tap-download) 找到 TapSDK Unity 下载地址，下载 TapSDK-UnityPackage.zip 然后解压，导入其中的 `TapTap_Common`，`TapTap_TapDB` 和 `TapTap_Themis` 模块。
