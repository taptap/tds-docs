...
您可以自定义处理点击通知栏消息通知的动作，包括：打开应用首页、打开特定URL和打开自定义页面。

打开应用自定义页面有两种方式，一种是通过服务端 API 指定 intent 参数，另一种是指定 action 参数，我们混合推送 SDK 选择使用 intent 参数（具体可参考荣耀文档 [服务端发送 push 消息 ](https://developer.hihonor.com/cn/kitdoc?category=%E5%9F%BA%E7%A1%80%E6%9C%8D%E5%8A%A1&kitId=11002&navigation=ref&docId=downlink-message.md&token=)），开发者也可以在发送请求时自行指定 action 参数（按照荣耀的 REST API 规范指定即可）。
