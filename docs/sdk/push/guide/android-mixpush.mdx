...
<ul>
  <li>
    华为（HMS){" "}
    <code>cn.leancloud:mixpush-hms:{sdkVersions.leancloud.java}</code>
  </li>
  <li>
...
1. **注册魅族账号**：在 [Flyme 开放平台](https://open.flyme.cn) 上注册魅族开发者账号并完成开发者认证（[详细流程](http://open-wiki.flyme.cn/doc-wiki/index#id?8)）。
...
注意：OPPO 推送服务 SDK 3.0.0 版本支持的最低安卓版本为 Android 4.4 系统（`minSdkVersion="19"`）。
