...
:::info
如果希望使用 Node.js 编写简单的云函数或 Hook，也可以尝试 [云函数和 Hook 开发指南 § 在线编写云函数](/v3/sdk/engine/functions/guides/#在线编写云函数)。
:::
...
<p>
  <code>{CLI_BINARY} up</code> 同时默认在 3001
  端口启动了一个用于调试云函数的控制台（
  <a href="http://localhost:3001">http://localhost:3001</a>
  ），开发者可以通过这个控制台来调试云函数和 Hook，模拟特定的输入。
</p>
...
接下来可以查看 [云函数和 Hook 开发指南](/v3/sdk/engine/functions/guides) 来了解云函数开发的详细信息、查看 [云引擎 SDK 使用指南](/v3/sdk/engine/functions/sdk) 了解 SDK 的进阶用法、查看 [云引擎平台功能](/v3/sdk/engine/platform) 来了解云引擎提供的更多功能，或查看专门的页面来了解具体运行环境的详情：
