...
LeanDB Elasticsearch 是云引擎提供的托管数据库，开发者可以在云引擎中使用 Elasticsearch 客户端类库或 HTTP API 连接，访问完整的 Elasticsearch 功能，更多其他托管数据库请查看 [云引擎服务总览](/v3/sdk/engine/overview)。
...
开发者可以在 **<Path to="engine" /> > 数据库 > Elasticsearch** 页面创建和管理 LeanDB Elasticsearch 实例。
