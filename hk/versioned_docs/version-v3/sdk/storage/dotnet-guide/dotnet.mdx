...
请阅读 [数据存储 .NET SDK 配置指南](/v3/sdk/storage/guide/setup-dotnet/)。
...
`Select` 支持点号（`author.firstName`），详见[《点号使用指南》](/v3/sdk/storage/guide/dot-notation/)。
...
保存文件时，SDK 会自动将文件大小写入元信息的 `size` 字段，开发者无需手动指定 `size` 字段。
如果开发者通过 AddMetaData 手动指定了文件大小，那么 SDK 会将手动指定的 `size` 值保存到云端。
...
SDK 没有提供专门的下载文件的接口。
如前所述，文件保存到云端后，开发者可以获取到文件的 URL，可以据此自行实现文件下载功能。
...
若要限制结果和给定地点之间的距离，可以参考 API 文档中的参数。
