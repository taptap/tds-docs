...
1. 接入 TDS 公告服务 SDK 之前，请参考[《功能介绍》文档](../features/#功能介绍)完成**公告系统开通<Conditional region="cn">、域名配置</Conditional>**。
...
githubDeps={[
    ...
    {package: "com.taptap.tds.billboard", url: `https://github.com/TapTap/TapBillboard-Unity.git#${v3SDKVersions.taptap.unity}`},
...
dependencies {  
    ...
    implementation (name:'TapBillboard_${v3SDKVersions.taptap.android}', ext:'aar') // TapTap 公告系统
...
如果你已经完成[内建账户系统 Tap 登录](/v3/sdk/taptap-login/guide/start/#sdk-初始化)的初始化，这里只需要引入公告模块，然后添加下面初始化代码中高亮的部分到项目中即可。
...
* 初始化需要两个域名，分别是 API 域名和公告域名。参考文档关于[域名](/v3/sdk/domain/guide/)的说明，在 **开发者中心 > 你的游戏 > 游戏服务 > 应用配置 > 域名配置** 处完成配置之后填入。

</Conditional>

* `dimensionSet` 参考[公告系统维度配置](../features/#维度配置)，这里使用了两个预设维度：渠道（platform）和地区（location）。
...
当[游戏发布的公告配置的跳转链接为「游戏内模块」](../features/#发布公告)，或者游戏正文添加了链接并且链接类型为「游戏内模块」的时候，可以通过监听自定义事件来获取消息，并做相应处理。
...
REST API 请求的 Base URL（下文 curl 示例中用 `{{host}}` 表示）即应用的 API <Conditional region='cn'>自定义</Conditional>域名，可以在控制台<Conditional region='cn'>绑定、</Conditional>查看。<Conditional region='cn'>详见文档关于[域名](/v3/sdk/domain/guide/)的说明。</Conditional>
...
可以参考视频教程：[如何接入 TapTap 公告服务](https://www.bilibili.com/video/BV1tp4y1L7Fe/)，了解如何在 Untiy 项目中接入公告功能。
