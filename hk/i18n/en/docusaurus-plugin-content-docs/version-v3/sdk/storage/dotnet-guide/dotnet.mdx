...
See [Installing .NET SDK](/v3/sdk/storage/guide/setup-dotnet/).
...
To learn about how you can protect the data stored on the cloud, see [Data Security](/v3/sdk/storage/guide/security/).
...
Removing data from the cloud should always be dealt with great caution as it may lead to non-recoverable data loss. We strongly advise that you read [ACL Guide](/v3/sdk/storage/guide/acl/) to understand the risks thoroughly. You should also consider implementing class-level, object-level, and field-level permissions for your classes in the cloud to guard against unauthorized data operations.
...
To use LiveQuery in your app, go to **<Path to="storage" /> > Settings** and check the **Enable LiveQuery** option under the **Security** section.
...
A `create` event will be triggered when a new `LCObject` is created and fulfills the `LCQuery` you subscribed to. The `obj` is the new `LCObject` being created:
...
An `update` event will be triggered when an existing `LCObject` fulfilling the `LCQuery` you subscribed to is updated. The `obj` is the `LCObject` being updated:
...
An `enter` event will be triggered when an existing `LCObject`'s old value does not fulfill the `LCQuery` you subscribed to but its new value does. The `obj` is the `LCObject` entering the `LCQuery` and its content is the latest value of it:
...
A `leave` event will be triggered when an existing `LCObject`'s old value fulfills the `LCQuery` you subscribed to but its new value does not. The `obj` is the `LCObject` leaving the `LCQuery` and its content is the latest value of it:
...
A `delete` event will be triggered when an existing `LCObject` fulfilling the `LCQuery` you subscribed to is deleted. The `objId` is the `objectId` of the `LCObject` being deleted:
