...
<CodeBlock className="xml">
  {`<dependency org="cn.leancloud" name="storage-core" rev="${v3SDKVersions.leancloud.java}" />`}
</CodeBlock>
...
<CodeBlock className="scala">
  {`libraryDependencies += "cn.leancloud" %% "storage-core" % "${v3SDKVersions.leancloud.java}"`}
</CodeBlock>
