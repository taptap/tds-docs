...
int resultCode = new TapTapShareBuilder()
    ...
    .addHashtagIds("hashtag ids") // HashTag和活动Id
    .addGroupLabelId("group ids") // 论坛标签Id
...
switch (resultCode){
    ...
    case TapTapShareCode.Error_NotInstall: // 分享失败：未安装TapTap应用
    default: // 分享失败：TapTap应用不支持
}
...
val resultCode: Int = TapTapShareBuilder()
    .addTitle(title) // 分享标题
    .addContents(content) // 分享内容
    .addHashtagIds(hashtag) // HashTag和活动Id
    .addGroupLabelId(group) // 论坛标签Id
    .addFailUrl("fail url") // 未安装taptap应用时跳转链接
...
when (resultCode) {
    ...
    TapTapShareCode.Error_NotInstall -> // 分享失败: 未安装TapTap应用
    else -> // 分享失败: TapTap应用不支持
}
```
</>
...
## 测试
为了保证上线后，游戏对于用户是否正常使用Tap分享功能，请务必按照以下说明完成自测。
