...
检查是否是contents字段携带的文本数据里过大导致，Android系统对于intent传递的数据内容大小是有限制的。通常是1Mb。

## 如何判断当前TapTap版本是否支持分享？
...
if (TapTapShareUtil.checkTapTapInstall(this)
    ...
    // 当前已安装TapTap 且 支持分享功能
}
```

## 已经安装最新版TapTap，TapTapShareUtil.checkTapTapInstall(this)还是返回-1，提示没有安装。

检查当前项目SDK版本Android 11（API 30）以后需要再AndroidManifest.xml添加安装包可见性权限:
