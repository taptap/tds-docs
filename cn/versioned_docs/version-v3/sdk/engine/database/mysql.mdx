...
LeanDB MySQL 是云引擎提供的托管数据库，开发者可以在云引擎中使用 MySQL 客户端类库连接，访问完整的 MySQL 功能，更多其他托管数据库请查看 [云引擎服务总览](/v3/sdk/engine/overview)。

## 创建和管理实例

开发者可以在 **<Path to="engine" /> > 数据库 > MySQL** 页面创建和管理 LeanDB MySQL 实例。
...
- 更多的用法请参考 [MySQL Connector .NET 文档](https://dev.mysql.com/doc/connector-net/en/)
