...
:::info
这篇文档是针对 Java 运行环境的深入介绍，如希望快速地开始使用云引擎，请查看 [快速开始部署云引擎应用](/v3/sdk/engine/deploy/getting-started)。
:::
...
- [Servlet 示例项目](https://github.com/leancloud/servlet-getting-started)
...
我们建议 Java 项目至少选用 512 MB 以上的内存，Spring Boot 项目至少选用 1024 MB 以上的内存，并在之后的运行过程中根据内存用量统计随时调整。调整内存规格的方法详见 [云引擎平台功能 § 调整实例规格和数量](/v3/sdk/engine/platform/#调整实例规格和数量)。
:::
...
<Conditional brand="leancloud">对于新创建的应用，</Conditional>如未设置 Java
版本，云引擎会默认使用支持的版本中最新的稳定版本（LTS）。
<Conditional brand="leancloud">
  在 2021-09-02 之前创建的分组因兼容考虑会默认使用 Java `8`。
</Conditional>
:::
...
云引擎会根据项目根目录是否存在 `pom.xml` 或 `gradlew` 文件来判断所使用的包管理器，然后使用对应的 Maven 或 Gradle 命令来安装依赖并构建。
