...
更多关于云引擎开发的内容，请参考[云引擎服务总览](/v3/sdk/engine/overview/)。
...
默认部署备注为「从命令行工具构建」，显示在 **<Path to="engine" /> > 管理部署 > 云引擎分组 > 日志** 中。你可以通过 `-m` 选项来自定义部署的备注信息：
...
- 设置 git repo 地址以及 deploy key 的方法可以参考 [云引擎平台功能 § Git 部署](/v3/sdk/engine/platform/#git-部署)。
