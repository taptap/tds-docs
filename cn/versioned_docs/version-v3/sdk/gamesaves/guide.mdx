...
阅读此文档前请先阅读[云存档功能介绍](/v3/sdk/gamesaves/features/)，了解云存档的核心概念及功能。
## 权限说明
...
1. 绑定[域名](/v3/sdk/start/get-ready/#域名)，包括 API 域名和文件域名。

2. 云存档需要依赖于内建账户，因此需要通过开发者中心开通 **TDS 内建账户**。游戏存档会绑定在 TDSUser 下，所以请首先参考[内建账户系统](/v3/sdk/authentication/guide/)集成文档完成[ SDK 获取](/v3/sdk/authentication/guide/#sdk-获取) 、[SDK 初始化](/v3/sdk/authentication/guide/#sdk-初始化)以及登录功能的接入。
...
上面的例子中，存档元信息字段的含义请参考[云存档保存的游戏元数据](/v3/sdk/gamesaves/features#云存档保存的游戏元数据)。
...
查询条件的构造请参考[数据存储指南查询章节的说明](/v3/sdk/storage/guide/dotnet/)。
...
查询条件的构造请参考[数据存储指南查询章节的说明](/v3/sdk/storage/guide/java/)。
...
详见文档关于[应用凭证](/v3/sdk/storage/guide/setup-dotnet#应用凭证)的说明。
...
详见文档关于[域名](/v3/sdk/storage/guide/setup-dotnet#域名)的说明。
...
各个字段的含义请参考[云存档保存的游戏元数据](/v3/sdk/gamesaves/features#云存档保存的游戏元数据)。
...
`where` 的用法详见[数据存储 REST API 指南](/v3/sdk/storage/guide/rest#查询约束)

### 添加存档

添加存档时的必填字段和可选字段请参考[云存档保存的游戏元数据](/v3/sdk/gamesaves/features#云存档保存的游戏元数据)。

在调用添加存档接口前，请先创建 `gameFile` 和 `cover` 指向的[文件](/v3/sdk/storage/guide/rest/#创建文件)，并**确保文件的 ACL 为仅限当前用户读取。**
...
参见[有条件删除对象](/v3/sdk/storage/guide/rest#有条件删除对象)。
...
这些文件需要另外[删除](/v3/sdk/storage/guide/rest#删除文件)。
