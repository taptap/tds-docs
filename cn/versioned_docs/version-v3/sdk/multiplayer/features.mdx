...
多人在线对战是 <Conditional brand="tds">TDS</Conditional><Conditional brand="leancloud">LeanCloud</Conditional> 专门针对多人在线类型的游戏推出的后端服务。开发者不需要自己搭建后端系统，利用云服务就可以轻松实现游戏内玩家匹配、在线对战消息同步等功能。
...
- **游戏逻辑运算**：在线对战提供了 [MasterClient](/v3/sdk/multiplayer/guide/js/#masterclient) 作为客户端主机控制游戏逻辑。游戏内的所有逻辑都交给 MasterClient 来判断运转，如果 MasterClient 意外掉线，我们会自动将网络状态最好的客户端切换为 MasterClient，确保游戏顺畅进行；开发者也可以选择在服务端编写游戏逻辑（服务端游戏逻辑支持尚在开发中）。
...
- [多人在线对战开发指南 · JavaScript](/v3/sdk/multiplayer/guide/js/)
- [多人在线对战开发指南 · C#](/v3/sdk/multiplayer/guide/cs/)
...
- API 域名在 **应用配置 > 域名配置 > API** 处查看，参考文档关于[域名](/v3/sdk/domain/guide/)的说明。
...
- 设置 [玩家掉线后的保留时间](/v3/sdk/multiplayer/guide/cs/#在房间内保留断线用户)，在有效时间内如果该玩家加回房间，房间内依然保留该玩家的自定义属性。
...
2、PlayerA 通过某种通信方式（例如 [即时通讯](/v3/sdk/im/features/)）将房间名称告诉 PlayerB。
...
PlayerA 通过某种通信方式（例如 [即时通讯](/v3/sdk/im/features/)）邀请 PlayerB，PlayerB 接受邀请。
...
PlayerA 通过某种通信方式（例如 [即时通讯](/v3/sdk/im/features/)）告诉 PlayerB 已经加入房间的 roomName，PlayerB 根据 roomName 加入房间。
...
PlayerA 创建房间后，通过某种通信方式（例如 [即时通讯](/v3/sdk/im/features/)）告诉 PlayerB 已经加入房间的 roomName，PlayerB 根据 roomName 加入房间。
...
更多匹配接口请参考房间匹配文档：[JavaScript](/v3/sdk/multiplayer/guide/js/#房间匹配)、[C#](/v3/sdk/multiplayer/guide/cs/#房间匹配)。

### 游戏中

#### 相关概念

- **MasterClient**：多人在线对战使用 [MasterClient](/v3/sdk/multiplayer/guide/js/#masterclient) 在客户端担任运算主机，由 MasterClient 来控制游戏逻辑，例如判定游戏开始还是结束、下一轮由谁操作、扣除玩家多少金币等等。
- **自定义属性**：自定义属性又分为 [房间自定义属性](/v3/sdk/multiplayer/guide/js/#房间自定义属性) 和 [玩家自定义属性](/v3/sdk/multiplayer/guide/js/#玩家自定义属性)。我们建议将游戏数据加入到自定义属性中，例如房间的当前地图、下注总金币、每个人的手牌等数据，这样当 MasterClient 转移时新的 MasterClient 可以拿到当前游戏的最新数据继续进行运算。
...
游戏中的大部分消息都发给 [MasterClient](/v3/sdk/multiplayer/guide/js/#masterclient)，由 MasterClient 运算后再判定下一步操作。假设有这样一个场景：玩家 A 跟牌完成后，告诉 MasterClient 跟牌完成，MasterClient 收到消息后通知所有人当前需要下一个玩家 B 操作。
...
- [JavaScript - 自定义事件](/v3/sdk/multiplayer/guide/js/#自定义事件)
- [C# - 自定义事件](/v3/sdk/multiplayer/guide/cs/#自定义事件)

#### 游戏中断线重连

如果 MasterClient 位于客户端，MasterClient 断线后，多人对战的服务会重新挑选其他成员成为新的 MasterClient，原来的 MasterClient 重新返回房间后会成为一名普通成员。具体请参考 [断线重连](/v3/sdk/multiplayer/guide/js/#断线重连)。
...
- [快速入门](/v3/sdk/multiplayer/start/js/)：快速接入多人在线对战，并运行一个小 Demo
- [多人在线对战开发指南 · JavaScript](/v3/sdk/multiplayer/guide/js/)：对多人在线对战的所有功能及接口进行详细介绍。

### C#

- [快速入门](/v3/sdk/multiplayer/start/cs/)：快速接入多人在线对战，并运行一个小 Demo
- [多人在线对战开发指南 · C#](/v3/sdk/multiplayer/guide/cs/)：对多人在线对战的所有功能及接口进行详细介绍。
