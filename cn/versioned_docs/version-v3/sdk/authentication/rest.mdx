...
:::tip
请参考 数据存储 REST API 文档中关于 [Base URL](/v3/sdk/storage/guide/rest/#base-url)、[请求格式](/v3/sdk/storage/guide/rest/#请求格式)、[响应格式](/v3/sdk/storage/guide/rest/#响应格式)的说明。
:::
...
详见[自动验证第三方平台授权信息](/v3/sdk/storage/guide/dotnet#自动验证第三方平台授权信息)。
...
- `platform`：unionId 对应的注册平台，可由应用自行指定，微信、QQ、微博平台建议设为[这些推荐的值](/v3/sdk/storage/guide/dotnet#该如何指定-unionidplatform)。
...
参见[扩展：接入*UnionID*体系，打通不同子产品的账号系统](/v3/sdk/storage/guide/dotnet#扩展：接入-unionid-体系，打通不同子产品的账号系统)。
