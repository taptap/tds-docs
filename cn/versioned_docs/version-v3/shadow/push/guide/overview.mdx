...
msg| |本次推送的消息内容，JSON 对象，详见[消息内容参数](/v3/sdk/push/guide/rest#消息内容参数)。
...
注意：我们只保留最近一周的推送记录，并会对过期的推送记录定时进行清理。推送记录清理和推送消息过期时间无关，也就是说即使推送记录被清理，没有过期的推送消息依然是有效的，目标用户依然是能够收到消息。推送过期时间设置请参考[推送 REST API 使用指南](/v3/sdk/push/guide/rest/)的《过期时间和定时推送》一节。

## Unity 推送

请阅读 [Unity 推送指南](/v3/sdk/push/guide/unity/)。

## Unreal 推送

请阅读 [Unreal 推送指南](/v3/sdk/push/guide/Unreal/)。

## iOS 推送

请阅读 [iOS 推送指南](/v3/sdk/push/guide/ios/)。
...
详见 [Android 混合推送指南](/v3/sdk/push/guide/android-mixpush/)。

如果想要使用推送服务自有通道推送，请阅读 [Android 推送指南](/v3/sdk/push/guide/android/)。

## 使用 REST API 推送消息

请阅读[推送 REST API 使用指南](/v3/sdk/push/guide/rest/)。
