...
<code>cn.leancloud:mixpush-android:{v3SDKVersions.leancloud.java}@aar</code>

从 6.5.1 版本开始，我们额外提供了单一厂商的推送 library，以支持不希望全部集成的产品之需求，新 library 与厂商的对应关系如下：

<ul>
  <li>华为（HMS) <code>cn.leancloud:mixpush-hms:{v3SDKVersions.leancloud.java}</code></li>
  <li>小米 <code>cn.leancloud:mixpush-xiaomi:{v3SDKVersions.leancloud.java}</code></li>
  <li>魅族 <code>cn.leancloud:mixpush-meizu:{v3SDKVersions.leancloud.java}</code></li>
  <li>OPPO <code>cn.leancloud:mixpush-oppo:{v3SDKVersions.leancloud.java}</code></li>
  <li>vivo <code>cn.leancloud:mixpush-vivo:{v3SDKVersions.leancloud.java}</code></li>
  <li>荣耀 <code>cn.leancloud:mixpush-honor:{v3SDKVersions.leancloud.java}</code></li>
</ul>
...
{`dependencies {
  //混合推送需要的包
  implementation 'cn.leancloud:mixpush-hms:${v3SDKVersions.leancloud.java}'
  //即时通信与推送需要的包
...
1. 在 application 的 `onCreate` 方法中调用 `LeanCloud.initialize` 完成初始化之后，进行混合推送 library 的初始化：
    ...
    这里参数 `profile` 的用法可以参考[推送 REST API 使用指南](/v3/sdk/push/guide/rest/)的《Android 混合推送多配置区分》一节。
...
{`dependencies {
  ...
  implementation 'cn.leancloud:mixpush-android:${v3SDKVersions.leancloud.java}'
  //即时通信与推送需要的包
  implementation 'cn.leancloud:realtime-android:${v3SDKVersions.leancloud.java}'
...
- 参数 `profile` 的用法可以参考 [推送 REST API 使用指南](/v3/sdk/push/guide/rest/)的《Android 混合推送多配置区分》一节。
...
{`dependencies {
  ...
  implementation 'cn.leancloud:mixpush-vivo:${v3SDKVersions.leancloud.java}'
  //即时通信与推送需要的包
  implementation 'cn.leancloud:realtime-android:${v3SDKVersions.leancloud.java}'
  implementation 'io.reactivex.rxjava2:rxandroid:2.1.0'
}`}
...
{`dependencies {
  ...
  implementation 'cn.leancloud:mixpush-oppo:${v3SDKVersions.leancloud.java}'
  //即时通信与推送需要的包
  implementation 'cn.leancloud:realtime-android:${v3SDKVersions.leancloud.java}'
  implementation 'io.reactivex.rxjava2:rxandroid:2.1.1'
...
{`dependencies {
  //混合推送需要的包
  implementation 'cn.leancloud:mixpush-honor:${v3SDKVersions.leancloud.java}'
  //即时通信与推送需要的包
  implementation 'cn.leancloud:realtime-android:${v3SDKVersions.leancloud.java}'
  implementation 'io.reactivex.rxjava2:rxandroid:2.1.0'\n
  implementation 'com.hihonor.mcs:push:7.0.41.301'
