...
阅读本文前请先[完成 SDK 初始化](/v3/sdk/friends/guide/)。
...
参见[内建账户系统文档](/v3/sdk/authentication/guide/#设置其他用户属性)。
...
富信息提供了 [REST API 接口](/v3/sdk/friends/guide/#富信息-rest-api)。
开发者可以自行编写程序或脚本调用这些接口在服务端进行管理性质的操作。

## 添加好友

可以通过指定[好友码](/v3/sdk/friends/mutual/#好友码)添加相应玩家为好友。
...
使用分享链接功能需要首先部署落地页网站。
落地页网站可以部署在[云引擎](/v3/sdk/engine/overview/)或其他支持部署纯静态网站的服务器上。
