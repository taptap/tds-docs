...
{`"dependencies":{
  ...
  "com.taptap.tds.rtc":"https://github.com/TapTap/TapRTC-Unity.git#${v3SDKVersions.taptap.unity}",
}`}
</CodeBlock>
</>
...
dependencies {  
    ... 
    implementation (name:'TapRTC_${v3SDKVersions.taptap.rtc}', ext:'aar')
}`}
</CodeBlock>
</>
...
* `ClientId`、`ClientToken`、`ServerUrl`，参见关于[应用凭证](/v3/sdk/storage/guide/setup-dotnet#应用凭证)的说明。
...
```objc
// Objective-C SDK 在进入范围语音房间时会自动把音质设为 LOW
```
...
开发者可以在回调的 Header 中拿到 `sign` 字段，以便校验请求是否来自实时语音服务云端。
...
REST API 请求的 Base URL（下文 curl 示例中用 `{{host}}` 表示）即应用的 API <Conditional region='cn'>自定义</Conditional>域名，可以在控制台<Conditional region='cn'>绑定、</Conditional>查看。<Conditional region='cn'>详见文档关于[域名](/v3/sdk/storage/guide/setup-dotnet#域名)的说明。</Conditional>
