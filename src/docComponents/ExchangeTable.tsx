...
export const ExchangeTable = ({}) => {
  ...
  const currentExchangeData =
    exchangeData[currentLocale] || exchangeData["zh-Hans"];
  ...
  return (
    <table>
      <thead>
        <tr>
          <th>
            <Translate id="exchange.currency">币种</Translate>
          </th>
          <th>
            <Translate id="exchange.currencyType">
              货币类型（currency_type）
            </Translate>
          </th>
          <th>
            <Translate id="exchange.currentRate">
              实时汇率（本位币:美元 USD）
            </Translate>
          </th>
        </tr>
      ...
      <tbody>
        <tr>
          <td colSpan={3}>
            <Translate id="exchange.commonCurrent">常用货币</Translate>
          </td>
        </tr>
        {commonCurrencyList.map((currency) => (
          <tr key={currency}>
            <td>{currentExchangeData.common[currency]}</td>
            <td>{currency}</td>
            <td>{exchangeMap[currency] || "-"}</td>
          </tr>
        ))}
      </tbody>
    </table>
