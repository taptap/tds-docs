...
_notificationChannel | Optional | For Android 8.0 or higher, you need to pass the channel id to receive pushes properly. Please refer to the "Adapting for Android 8.0" section in the [Android Push Guide](/shadow/push/guide/android/).
...
The push record object will be returned. Refer to the section "Notification" in [Push Notification Overview](/shadow/push/guide/overview/) for the meaning of each field of the push record.

## Viewing Push Status and Canceling Pushes

During the process of sending a push, we will update the push status as the push task is executed in **Developer Center > Your game > Game Services > Cloud Services > Push Notification > Push records**, where you can check the latest status of the push. For a description of the different push statuses, please refer to the Notification section of the [Push Notification Overview](/shadow/push/guide/overview/).
