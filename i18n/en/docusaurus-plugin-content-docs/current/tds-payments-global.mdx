...
Goods type introduction:

| Type | Description |
| --- | --- |
| Consumable goods | **Consumable goods** refer to goods that your game will allocate the associated game content when the user consumes them, and then the user can purchase them again. For example, game currency, game props, etc. |  
| Non-consumable goods | **Non-consumable goods** refer to goods that can be permanently used after one purchase. For example, paid upgrades and chapter packs. |   

With the TapTap Developer Center, you can easily create in-game goods and seamlessly access TapPayments-Global's services in your game to sell game goods.
...
1. Refer to [Preparation](/sdk/start/get-ready/) to create an app, enable app configuration, and bind API domain name;
2. Refer to [Quick Start](/sdk/start/quickstart/#配置签名证书) to configure package name and signature certificate;
...
1. On the [Download](/tap-download) page, find the **TapSDK Unity** and **LeanCloud C# SDK** download links and download TapSDK-UnityPackage.zip and LeanCloud-SDK-Realtime-Unity.zip respectively.
...
If you have completed the initialization by following the Quick Start, you only need to introduce the payment module here.
...
var config = new TapConfig.Builder()  
    ...
    .ServerURL(serverUrl) // Required, Developer Center > Your Game > Game Service > Basic Information > Domain Name Configuration > API  
...
TapConfig config = new TapConfig.Builder()  
    ...
    .withServerUrl(serverUrl) // Required, Developer Center > Your Game > Game Service > Basic Information > Domain Name Configuration > API  
...
After the game accesses the TapTap SDK through Unity, you can initialize the payment module through the following methods:
...
{  
    ...
    {  
        ...
        {  
            Console.WriteLine ( "No relevant items were queried" );  
        }  
        else  
        {  
...
* `String language`: The required internationalization language code, such as: zh_CN, en_US, ja_JP, etc. If you do not need to customize the language country or use TapSDK for unified settings, you can use the language-free version of the function, and the SDK will automatically select the corresponding [language](#internationalization-support) based on the user's country or region. 
* `String[] itemIds`: The array of item IDs to be queried. The item ID is the ID of the item defined by the game in **Developer Center > Your Game > Game Services > TapTap Payments > Items and Orders**. <code>[Item](#item-information-item)</code> is the callback result of the queried existing item information.  
* &zwnj;<code>[Callback](#callback-callbackt)&lt;[Item](#item-information-item)&gt;[] callback</code>: Callback Object
  - Successful callback type: array of [Item](#item-information-item) item information: <code>(Array&lt;[Item](#item-information-item)&gt; items)</code>
  - Failure callback type: exception [Error](#exception-error): <code>([Error](#exception-error) error)</code>
...
:::tip Pass arguments description
The order information contains the order ID. You can pass in the order ID directly, or you can only pass in the order ID. When the query is successful, the status field <code>[[State](#order-state-orderstate) state]</code> of the order information <code>([Order](#order-order) order)</code>passed in <code>[Callback](#callback-callbackt)&lt;[Order](#订单-order)&gt; callback</code> may be updated.
:::
...
public class Callback < T > 
{
    ...
    /// When an error occurs, error information. For details, see <see cref="Error"/>.  
    /// </param>  
...
Use the two-letter lowercase language codes defined in ISO 639-1 (for example, `en` indicates English, `jp` indicates Japanese), but:

* Due to historical reasons in the Android system, Indonesian uses `in` instead of `id`. 
...
In addition, when setting item names in the DC backend, multi-language is also supported. The passed language parameters will affect the return of the corresponding internationalized item name.
...
#### Verify Order
After the payment is successful, verifying the order indicates that the payment result has been confirmed and the shipment will be completed for the buyer. The order status will change from `charge.succeeded` to `charge.confirmed`.
...
|charge.overdue     | Payment Timeout Closed      | 
...
Currently, Webhook only monitors the "Refund Successful" event: `refund.succeeded`. For successful payment, it is recommended to take the initiative to `verify the order` and complete the shipment according to the order status.
...
According to the above header, `ts = 1687224754` is obtained. You can first confirm whether the difference from the current system time is too large to ensure that it is not an old reused request.
The request payload is a JSON string defined as `payload`. （
...
You also need `webhookToken` configured in the DC backend as the secret of the HmacSHA256 algorithm to finally get

```
sign = HmacSHA256(secret, ts + "." + payload ) 
```

Compare whether the calculation result sign is the same as the signature part of `TapPay-Signature`
