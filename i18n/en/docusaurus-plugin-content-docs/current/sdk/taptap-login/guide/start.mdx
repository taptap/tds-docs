...
If the player logs out of the game or clears the game's cache, then the login info will also be deleted. Next time the player logs in, `TDSUser#currentUser` will return null.
