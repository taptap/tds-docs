...
When using the [Basic TapTap User Verification Interface](/sdk/taptap-login/guide/tap-login/), an access token bundle is generated after the user granting permission to the current application. It can be transformed into a MAC Token string after encryption. This access token bundle remains valid until the user updates their account's security information or denies authorization. Developers are advised to manage MAC Tokens on their server as a means of subsequent communication with the TapTap server.
...
| invalid_client        | Invalid client_id and/or client_secret parameters                             |
