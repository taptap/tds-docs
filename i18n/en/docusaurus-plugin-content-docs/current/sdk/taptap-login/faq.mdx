...
TapTap Login's authorisation information will trigger an authorisation expiration and automatically clear the data when the player enters the game without opening it for 30 days (without triggering an initialisation).
You can listen to this value and if it is cleared, go through the [initialisation](/sdk/taptap-login/guide/start/#quick-log-in-with-taptap) process again and log in again, the Openid value will not change when logging in again.