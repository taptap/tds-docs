...
Please create an account on [Zhongxuanbu’s anti-addiction and real-name verification system for online games](https://wlc.nppa.gov.cn/fcm_company/index.html#/login?redirect=%2F) and confirm that the interfaces they provide are functional. You will get the following credentials from the system:
...
The CAC review status of the game must be `Approved`.

![](https://capacity-files.lcfile.com/BC7gUR6wfpOh9PHj8XwKybxseeHhl6Fq/anti-examine-success.png)

The game must complete the interface test in the CAC, the CAC currently needs to test 8 interface use cases, it should be noted that in the CAC `Test Interfaces` > `Preferences` > `IP Whitelist` here the `IP Whitelist` must be configured with the game side of the public IP, the CAC interface test is temporary and after it is passed it will no longer be used.

![](/img/anti-addiction/testcase.png)

Then complete the parameter configuration in the TapTap Developer Center backend. Use the IP whitelist address provided in the TapTap Developer Center backend to fill in the CAC system backend:
...
Once you have finished the steps above, you will be able to start integrating TDS’ Real-Name Verification and Anti-addiction service into your game. Once the application has been registered, you will need to complete the CAC's "interface test" and the game will need to pass the CAC's audit.

The service consists of two portions:
