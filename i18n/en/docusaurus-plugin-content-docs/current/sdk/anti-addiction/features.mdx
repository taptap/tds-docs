...
## Test Accounts
There are two types of scenarios for using test account:

- The first is when the game is in development and testing, and the test account is needed to test the real name authentication anti-obsession feature.
- The second is when you are applying for the game version number.

Real-name authentication anti-sedentary provided by the test account is strongly dependent on the TapTap login, in accordance with the requirements of the State Press and Publication Administration and therefore provides 30 accounts, including:

- One group of empty accounts without real names, totalling 3;
- Two groups of minor accounts, 18 in total, each containing 3 each of high, medium and low (16 years of age and under 18 years of age, 8 years of age and under 16 years of age, and 8 years of age and under 8 years of age);
- One group of adult accounts (over 18 years old), with a total of 9 accounts.

According to the usage scenario of the test account, the usage mode of the test account is divided into two types: **Regular Mode** and **Test Mode**.

### Regular Mode

The status and attributes of the test account cannot be changed in the regular mode, which is applicable to the scenario of submitting the game to the moderation office for review and applying for a version number.

### Test Mode

To facilitate developers to test the real-name authentication anti-addiction function for underage players, a test mode is introduced, under which the status and attributes of the test account can be changed. For example, for the underage test account provided, you can change whether the current time is playable or not, and the amount of money that has been recharged, so that the developer does not need to wait for the underage to comply with the time period of the game to test the anti-addiction function. Test accounts can be viewed by going to **TapTap Developer Centre > Your Games > Game Services > Compliance Certification > Test Accounts** in order.