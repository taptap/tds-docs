...
:::tip
Before using the TDS’ Real-Name Verification and Anti-addiction service, please first enable the service on **开发者中心后台 > 游戏服务 > 生态服务 > 合规认证**. When enabling the service, you can select whether your game has gotten an ISBN.
:::
...
The SDK supports iOS 10 and higher. The SDK is compiled with Xcode 12.
...
2. Manual entry: This lets players enter their identity information with the user interface provided by the SDK.
...
- **[Add testers](/sdk/start/test-accounts/) on the Developer Center** or open up the game to everyone. If this step is not completed, the player will see “抱歉，该游戏 / 应用暂未开放” when using TapTap Quick Verification.
:::
...
The body of each request sent to the REST API shall be a JSON object. The `Content-Type` in the HTTP header should be `application/json`.
...
You can enable the service on **开发者中心后台 > 游戏服务 > 生态服务 > 合规认证**. You can choose one of the following two options for your game:
...
If you see the “授权失败” error when using TapTap Quick Verification, please [configure the signature certificate](/sdk/start/quickstart/#configure-signature-certificate) on the Developer Center.

### 未查询到实名认证配置 

If you see the “未查询到实名认证配置” error, it is because the Real-Name Verification service is not enabled. Please enable it on 开发者中心后台 > 游戏服务 > 生态服务 > 合规认证.
...
* If your game **already has an ISBN**, please make sure the parameters entered on 开发者中心后台游戏服务 > 生态服务 > 合规认证 are correct. If the parameters are incorrect, the service will be unable to make requests to Zhongxuanbu’s interface and players may be asked to verify their identity repetitively. Please check the following parameters:
