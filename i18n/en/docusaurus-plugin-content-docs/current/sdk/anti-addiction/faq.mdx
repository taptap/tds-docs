...
### Use TapTap Quick Authentication: Could not get real name information, please try again later.

- As of v3.22.0, TapTap Quick Authentication will not report this exception if the account logged in by the TapTap client is not a real name in the TapTap client, but will jump to the TapTap client for real name authentication.
- Check that the device time is synchronised with the network, an inaccurate device time will also cause this exception.