...
### Calling the gift redemption interface reports an exception: {"error":100016,"message":"该礼包码无效码","info":{"dev_message":"invalid code","hint":"gift code is invalid"}} 。

Check if the gift code passed in the interface is wrong, the gift code can be found in **Tap Developer Center > Your Game > Operational Tools > Gifts > Gift Event > Data > Export**, after clicking the `Export` button a `.csv` file will be generated, and you can check the gift code in that file.

### Calling `GameSend` gift redemption interface reports an exception: {"error":100015,"message":"发送道具失败"} 。
