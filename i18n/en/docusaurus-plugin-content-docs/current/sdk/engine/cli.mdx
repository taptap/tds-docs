...
The default message for the deployment is “Built from lean-cli”, which will be displayed on **<Path to="engine" /> > Your group > Logs**. You can customize the message with `-m`:
...
- See [Cloud Engine Platform Features § Deploying With Git](/sdk/engine/deploy/platform/#deploying-with-git) for instructions on how to set up Git repository and deploy key.
