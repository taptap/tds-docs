...
## When using file upload in Cloud Engine, the error message is: failed to upload file to qiniu bcz current file has not data stream。
...
1. The permissions issue mentioned above will occur. This makes it impossible to create cache directories and temporary files in the cloud engine.
2. All file data goes to the server first, and then the API is called to push it to the file storage provider, which leads to low efficiency and high network cost for the user (the cloud engine also needs to pay for more than the standard traffic cost).

It is recommended to use the SDK to upload the file, and then give the result (e.g. the object id or url of the file) to the server (which is the cloud engine code). Uploading on the client side will use the file storage provider's CDN link (nearby uploads), which makes the user experience faster and saves the cloud engine's traffic costs.
