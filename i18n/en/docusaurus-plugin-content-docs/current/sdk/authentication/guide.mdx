...
:::tip
We recommend that you store users’ nicknames with the `nickname` field because [TDS’s Friends module](/shadow/friends/guide/) **uses the data in this field when looking up friends with nicknames or generating invitation links**.
