...
You might be tempted to store all the data related to users, like their game levels and the items they own, in the account system. However, this is not encouraged and not supported.
...
Therefore, we restrict the use of fields. It’s totally fine to store a small amount of data that’s only accessible to its owners in the user account,
