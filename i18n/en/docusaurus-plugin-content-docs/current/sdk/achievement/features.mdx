...
=======
To add testers and have them test the achievements with the **Ready** status, please contact the TDS technical support team by opening a ticket on the Developer Center. "In the work order, please provide the `Client ID` of the application and the `Object ID` returned by the test account after implementing the TapTap login.
>>>>>>> 3f57a88c (docs: achievement system opening test account instructions)
:::
