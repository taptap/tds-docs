...
### I see the error message "抱歉，该游戏/应用暂未开放"

You will see this exception if you haven't clicked "Make available for all" in **Game Services > Gaming Ecosystem > TapTap Login**, or if you try to test services including TapTap Login and Copyright Verification without [adding test users](/sdk/start/test-accounts/).

Solution:

* If the game is still **under test**, you can select "Manage Testers" and click the "Add User" button. Search by user ID or nickname to add them to the test user list.
* If the game is already launched, click "Make available for all".
