...
Log into the <Conditional region="cn">[TapTap Developer Center](https://developer.taptap.com/)</Conditional><Conditional region="global">[TapTap Developer Center](https://developer.taptap.io/)</Conditional> to register as a developer and create a game.
...
After importing the SDK, please continue with the following instructions to get your project ready for Android and iOS.
...
2. Add the following lines into the `application` tag in `Assets/Plugins/Android/AndroidManifest.xml`:
...
1. [Download TapSDK_Android](/tap-download) and import it to `project/app/libs`.
...
1. Select the project in Xcode and add `-ObjC` into **Build Setting > Other Linker Flags**.
...
You can use [**GenSignatureMD5**](https://blog.taptap.dev/tools/get-signature-md5.apk) to obtain the MD5 hash of your game.
