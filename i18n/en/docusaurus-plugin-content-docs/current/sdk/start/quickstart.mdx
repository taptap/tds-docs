...
The [Downloads](/tap-download) page provides Unity, Android, iOS demos for your reference.
...
2. Inside the Unity project, navigate to **Assets > Import Packages > Custom Packages** and select the TapSDK packages you want to use from the unzipped `TapSDK-UnityPackage.zip` file. Of which:
...
After importing the SDK, please complete the following configurations related to Android and iOS.
...
4. Add additional configurations for older Android versions.
   ...
   - Add `xmlns:tools="http://schemas.android.com/tools"` for `manifest`
   - Add `tools:remove="android:requestLegacyExternalStorage"` for `application`
