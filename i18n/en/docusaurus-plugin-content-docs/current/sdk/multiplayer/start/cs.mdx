...
The SDK can be imported via Unity Package Manager or manually, either one of them:
...
When a new player joins the room, the Master assigns a score to each player, which is synchronised to the player via "Player Custom Properties".
