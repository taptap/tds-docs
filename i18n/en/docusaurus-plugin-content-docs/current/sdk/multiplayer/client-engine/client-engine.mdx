...
:::info
**Before reading this document, please read the [Multiplayer Service Features](/sdk/multiplayer/features/) and [MasterClient](/sdk/multiplayer/guide/js/#masterclient) to understand the infrastructure of Multiplayer development. **
:::
...
The system will charge according to the daily peak consumption of computing units, the price of a single computing unit can be found on the [official website](https://leancloud.cn/pricing/). For example, if an application on a domestic node uses up to 2 computing units on a given day, the charge for that day will be 2 * domestic computing unit price.
