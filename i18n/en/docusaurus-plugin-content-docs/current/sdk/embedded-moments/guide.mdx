...
Please first [download](/tap-download) the TapSDK and add the required dependencies. Embedded Moments depends on `TapLogin`, `TapCommon`, and `TapMoment`.

If you use TapSDK Unity v3.7.1 or higher version, make sure that you add `com.leancloud.storage` module.
