...
The Base URL for the REST API (represented with `{{host}}` in curl examples) is the API domain of your app. You can manage and view it on the Developer Center.
...
For requests that may trigger hooks on LeanEngine, use the `X-LC-Prod` HTTP header to specify the invocation environment:
