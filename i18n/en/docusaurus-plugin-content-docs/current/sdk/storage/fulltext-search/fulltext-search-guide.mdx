...
You can use `SearchSortBuilder` to build complex rules for sorting. For example, assuming `scores` is an array of scores, if you need to sort results in descending order of the average score, and put those who do not have scores at the last:
...
The word list will take effect in 3 minutes since you upload it. You can check if your word list is in effect with the [`analyze` API](/sdk/storage/guide/fulltext-search-rest#performing-analysis) (requires Master Key).
