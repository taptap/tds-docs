...
Push service is free to use, and does not take up the free quota, push message interface calls are subject to frequency limitations, refer to: [Push message interface limitations](/sdk/push/guide/rest/#push-message-interface) documentation.

<Conditional brand="leancloud">

By default, the maximum number of worker threads that can be used by the Business Edition application at any one time is 60, which means that at most 60 data requests can be processed at the same time. **We will adjust this value based on application performance and operation and maintenance needs**. If you need to increase this limit, please [submit a work order](https://www.leanticket.cn) to request it.
...
You can continue to get new results from the previous breakpoint by changing the query conditions each time, for example:
- First query, createdAt time after 2015-12-01 00:00:00 1000 pieces of data (the last createdAt value is x);
- Second query, 1000 data items with createdAt after x (the last item with createdAt is y);
- Second query, 1000 data items after createdAt x (last item createdAt is y); third query, 1000 data items after createdAt y (last item is z);

The third query, 1000 data items after createdAt y (the last item is z); and so on.

### Can paging be accomplished by specifying a range of objectId's?

The current algorithm for automatically generating `objectId` by the datastore service is time-based, so it is indeed possible to implement paging by specifying a range of `objectId`s.
...
As with the use of traditional databases, query optimization is achieved primarily by **indexing**. Indexes are like a table of contents in a dictionary, which can help you look up words more quickly in a huge amount of text.

Principle: When the amount of data is small, do not build indexes. When there is more, remember that after building an index, you still need to update the index when writing, in exchange for less query time. So, in general, write less read more to build more indexes, write more read less to build less indexes.
...
This is the limitation of default value, MongoDB itself does not support default value, the default value we provide is only the enhancement of the application level, old data such as there is no corresponding key, after setting the default value will not revise the data, but only after the query to do the presentation layer optimization. Accordingly, after changing the default value, these keys will also be displayed as the new default value. There are two solutions:

1. do an update to the old data, query the records where the key does not exist (whereDoesNotExist), and then update them back.
2. Add or query to the query condition, or key does not exist (whereDoesNotExist).

### How do I query for non-null values?

The exact semantics of a null value depends on the type of the field and the actual situation of the item. Taking a string field as an example, a null value may mean:
...
If the error message is similar to `can't find any special indices: 2d (needs index), 2dsphere (needs index), for field name`, it means that the field used for the query doesn't have a 2D index created, you can find **Indexes** management in the **Other** menu of the Class management. You can find **Index** Management in the **Other** menu of Class Management, click into it, find the field name, select and create "2dsphere" index type.

![image](/img/storage/geopoint_faq.png)

### How to improve query efficiency for flag bits?

When there are many Boolean types of data in the data table, you can consider using binary storage to improve the query efficiency. For example, if you need to store multiple states such as whether to turn on push, mute, be a member, etc., you can represent it like this:

`111`: enable push, mute, member

`101`: push on, not muted, is a member
...
- Select the required field (don't select data field) at the time of query, and fetch the data field to the client only at the time of GET object.
- Or consider saving the data in the data field as a "file" and associating it with a Pointer in a table.
- If the data is a huge object and you only need to get one or more of its attributes, consider specifying the attribute(s) with a dot in select, see [Use dot in querying objects](/sdk/storage/guide/dot-notation/#use-dot-in-querying-objects).

## File storage

### Is File Storage CDN-accelerated?

The file storage service of China node comes with CDN acceleration, but does not include overseas CDN acceleration.
...
There is no size limit for any of the SDKs, except for a 10 MB size limit for uploading files through the JavaScript SDK in a browser, or directly through our website. The JavaScript SDK also has no size limit in the Node.js environment.
...
If you are using an external file (constructed via URL) and need to modify the file or metadata frequently, you can consider the following scenario:

- Implement a class that stores information about the external file, such as `ExternalFile`. Set a Pointer field in the object that references the file to point to `ExternalFile`. Note that in this case, when querying the object containing the file, if you want to get the file information at the same time, you need to include the corresponding field (if you use the built-in `_File` Class, it will be included automatically, so you don't need to specify it separately).
- If you want to reference a file, you can use a string type to save the URL; in this case, you need to implement the file-related functions by yourself. If you want to save meta information about the file, you need to design it separately. If you want to save the meta information about the file, you need to design it differently, for example, by saving it on other fields of the object, or by changing the type of the corresponding field from string to object and saving both the URL and the meta information. Note that such a design will result in the application's file information being scattered among the classes that need to use the file, and it will be troublesome to batch-query and batch-process the entire application's files.

## SDK

### iOS project size after packaging

Create a brand new blank project, install AVOSCloud and AVOSCloudIM modules using CocoaPod, and the project size is over 80 MB. will it shrink after packaging? Will it shrink after packaging?

The datastore iOS SDK binary contains 5 CPU slices, i386, armv7, arm64, etc. Non-ARM symbols and symbols not involved in the connection are stripped out during the release process. Therefore, the final app size will not increase more than 10 MB, so please feel free to use it.
...
- The SDK version is too old, and the logic for generating the installationId has been changed in a version change. Please update to the latest version.
- Code obfuscation, please add [SDK obfuscation exclusion](/sdk/storage/guide/setup-java/#android-code-obfuscation) to the proguard file.
...
### What is the reason for the Java SDK already has one request sending error?
...
### Using the data storage feature of the Android SDK, the app was detected by the vendor to have a self-start issue when it was uploaded.

Workaround: Upgrade the Android SDK to v8.2.19 or later.

The reason for the self-start issue is that LiveQuery needs to have a long link in the background so that it can receive event notifications from the cloud in time, and this long link is placed in a background service with auto-reconnect capability - it will listen to the network status changes and start this service when the network is active. This logic may be recognized as "self-starting" by some platforms.

Older versions of the SDK sometimes threw exceptions because they didn't adapt to newer versions of Android, and version 8.2.19 of the SDK adapts to newer versions of Android, but it doesn't completely disable this auto-reconnect logic, so it may still be misinterpreted. If you have already used the new SDK and still encounter this problem, you can complain to the vendor.

### Does JavaScript SDK have a synchronization API?

The JavaScript SDK does not provide a synchronization API due to platform specificity (running in a single-threaded browser or Node.js environment), and all APIs that require network interaction need to be called as callbacks. We provide [Promise mode](/sdk/storage/guide/js/#promise) to minimize the problem of over-nested callbacks.

### JavaScript SDK uses Master Key in AV.init, but outgoing AJAX requests return 206
...
### The JavaScript SDK exposes the App Key and App Id, how can I ensure security?
...
Theoretically all clients are untrusted, so security needs to be engineered on the server side. If you need advanced security, you can use the [ACL](/sdk/storage/guide/acl/) method to manage it, and if you need more advanced customization, you can use the [cloud engine](/sdk/engine/overview/).

## Other

### How do I prevent DDoS attacks, or high-frequency CC attacks?
...
### How to solve the data consistency or transaction needs?
...
- Updates to multiple fields are done atomically in a single save operation on a single object.
- Numeric fields can be updated atomically using [increase](/sdk/storage/guide/js/#update-counter) (atomic counter).
...
- [Conditional update object](/sdk/storage/guide/js/#conditionally-update-object) can update only when a query condition is met; based on this feature, you can implement more complex [two-phase commit](http://www.howardliu.cn/translation-perform-two-phase-commits-in-mongodb/).
- You can also implement custom [exclusion locks](https://github.com/leancloud/leanengine-nodejs-demos/blob/master/) on the cloud engine with the help of [LeanCache](/sdk/engine/database/redis/). functions/redlock.js).

</Conditional>

We've also recorded a [public video](https://www.bilibili.com/video/av12823801/) on this topic, which has a detailed introduction to these features and example tutorials that address common scenarios (including implementing a two-phase commit).